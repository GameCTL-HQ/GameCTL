...
#       Storage GUI writes; merged-by-name, never overwrites GUI edits).
#       NFS:   name=1TBSSD,server=10.0.0.100,path=/mnt/1TBSSD
#       Local: name=local-ssd,type=local,path=/mnt/1TBSSD  (host path, single-node)
#       (';'-separated for multiple entries)
...
# BASH_SOURCE[0] is unset when the script is run via `curl … | bash` (no
# file on disk). Guard it under `set -u`, and leave SCRIPT_DIR empty in
# that case — the manifest lookup below will fall through to the URL.
_src="${BASH_SOURCE[0]:-}"
...
if ! kubectl cluster-info >/dev/null 2>&1; then
  ...
  # export $KUBECONFIG for this session AND persist it to ~/.bashrc so
  # the user doesn't hit this again on the next shell.
  if [ -e /etc/rancher/k3s/k3s.yaml ] && [ "$(id -u)" -ne 0 ]; then
   ...
   (or pass --write-kubeconfig-mode=644 at k3s install time)"
    ...
    # The k3s-wrapped kubectl ignores ~/.kube/config unless $KUBECONFIG
    # is set, so set it for the rest of this script's run…
    export KUBECONFIG="$HOME/.kube/config"
    ...
    if [ -f "$HOME/.bashrc" ] && [ -w "$HOME/.bashrc" ] \
      ...
      {
        printf '\n# Added by GameCTL installer — the k3s-wrapped kubectl\n'
    ...
    ok "Cluster reachable via \$KUBECONFIG=$KUBECONFIG"
  else
...
# Pin a moving :latest default to the newest immutable release tag. This stops
# unprompted version jumps: with imagePullPolicy: Always, a pod that restarts on
# a :latest tag re-pulls whatever :latest now points at — so a node drain or
# eviction silently updates GameCTL. Pinning a fixed tag means restarts re-pull
# the SAME version; only the in-app "Update" button (which sets a new tag) moves
# it. A user-supplied GAMECTL_IMAGE with an explicit tag is respected as-is.
case "$IMAGE" in
  *:latest)
    _repo="${IMAGE%:latest}"
    # GameCTL-HQ/GameCTL is the release repo (matches UpdateRepo in config.go).
    _tag="$(curl -fsSL --max-time 10 \
      https://api.github.com/repos/GameCTL-HQ/GameCTL/releases/latest 2>/dev/null \
    ...
    if [ -n "$_tag" ]; then
      ...
      warn "set GAMECTL_IMAGE=<repo>:<version> to pin explicitly."
    fi
    ;;
esac
...
if [ "$EXPOSE" = "auto" ]; then
  ...
  if [ "$ASSUME_YES" = "1" ]; then
    ...
    # Offer an Ingress IN ADDITION only for NodePort. Not for LoadBalancer:
    # picking MetalLB is an explicit "no DNS needed" choice, so re-asking
    # about a hostname there just contradicts what they picked.
...
echo
# Append ?token=… so the UI's setup form pre-fills the bootstrap token
# (handled by kubeUI/src/App.jsx → URLSearchParams on the initial setup
# state). Empty when no token was scraped (already-claimed install).
qs="${TOKEN:+?token=${TOKEN}}"
echo "  1) Open the UI:"
if [ "$EXPOSE" = "ingress" ]; then
  echo "       ${c_grn}http://${HOST}/${qs}${c_rst}   (point DNS/hosts for ${HOST} at your ingress controller)"
  ...
  echo "       • immediate access: kubectl -n ${NS} port-forward svc/gamectl 8080:8080  → http://127.0.0.1:8080/${qs}"
else
  [ -n "$UI_URL" ] && echo "       ${c_grn}${UI_URL%/}/${qs}${c_rst}"
  echo "       Always works: kubectl -n ${NS} port-forward svc/gamectl 8080:8080  → http://127.0.0.1:8080/${qs}"
fi
if [ "$ALSO_INGRESS" -eq 1 ]; then
  echo "       Also via Ingress: ${c_grn}http://${HOST}/${qs}${c_rst} (needs DNS → ingress controller)"
fi
echo "  2) Choose your admin username/password (token is pre-filled from the URL). Done."
echo "${c_blue}${bar}${c_rst}"
