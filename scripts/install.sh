...
if ! kubectl cluster-info >/dev/null 2>&1; then
  # Classic k3s footgun: /etc/rancher/k3s/k3s.yaml is owned root:root mode 600
  # by default, so kubectl as a normal user gets "permission denied" → no
  # context → "can't reach a cluster". If it looks like that, offer to copy
  # the kubeconfig into the user's $HOME/.kube/config (the standard fix).
  if [ -e /etc/rancher/k3s/k3s.yaml ] \
     && [ -z "${KUBECONFIG:-}" ] && [ ! -e "$HOME/.kube/config" ] \
     && [ "$(id -u)" -ne 0 ]; then
    ...
    if confirm_invasive "Copy it to ~/.kube/config (chown $USER, mode 0600)?"; then
      mkdir -p "$HOME/.kube"
      ...
      if ! kubectl cluster-info >/dev/null 2>&1; then
        die "still can't reach the cluster after the copy — check 'kubectl config current-context'"
      fi
    else
