...
export default function MetricsServerNotice({ variant = 'panel' }) {
  ...
  // st is null until the fetch resolves. Every other variant has already
  // returned by now, but `reference` renders unconditionally — so it reaches
  // this code with nothing loaded on first paint, and a bare st.installed
  // threw, white-screening the whole Monitoring page. Optional access +
  // FALLBACK_CMD means the command is on screen immediately and simply gets
  // more specific once the status arrives.
  ...
  if (reference) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        <h3 className="text-sm font-semibold text-slate-200">Can't see CPU/RAM metrics? Run this</h3>
