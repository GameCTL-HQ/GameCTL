...
function PhaseRow({ phase }) {
  ...
  return (
    <li className="grid grid-cols-[auto_1fr_auto] items-start gap-2 py-1.5 border-t border-slate-800 first:border-t-0">
      <span className={`h-2 w-2 rounded-full mt-1.5 ${s.dot}`} />
      <div className="min-w-0">
        <p className="text-xs text-slate-200 truncate" title={phase.name}>{phase.name}</p>
        {phase.detail && phase.status === 'running' && (
          <p className="text-[10px] text-slate-500 truncate">{phase.detail}</p>
        )}
        {phase.error && (
          <p className="text-[10px] text-rose-300 font-mono truncate" title={phase.error}>{phase.error}</p>
        )}
      </div>
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-slate-700 bg-slate-900 p-6 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-start gap-3 mb-4">
          ...
          {task && (task.status === 'running' || task.status === 'pending') && (
            <button
              onClick={async () => { setStopping(true); await cancelTask(taskId) }}
              disabled={stopping}
