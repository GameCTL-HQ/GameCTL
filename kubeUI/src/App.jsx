...
// SteamAccount import kept (page file retained) but not mounted — see the
// route/nav comments below. Restore alongside dayz-kube.
...
function parseHash() {
  ...
  if (section === 'stats')     return { view: 'stats',     selectedGame: null }
  if (section === 'monitoring') return { view: 'monitoring', selectedGame: null }
  // '#/steam' deliberately falls through to the hub while the Steam
  // credentials screen is disabled. Hiding only the nav button would still
  // leave a bookmarked or shared link asking for a Steam password.
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <header className="border-b border-slate-800 bg-slate-900 sticky top-0 z-20">
        <div className="max-w-[92rem] mx-auto px-4 py-3 flex items-center gap-3 sm:gap-4">
          <button onClick={() => { setView('hub'); setSelectedGame(null) }} className="shrink-0">
            <img src="/brand/gamectl-logo.png" alt="GameCTL" className="h-10 sm:h-16 w-auto" />
          </button>

          {/* Breadcrumb */}
          <nav className="flex items-center gap-1 text-sm text-slate-400 min-w-0">
            <button onClick={() => { setView('hub'); setSelectedGame(null) }} className="hover:text-slate-100 shrink-0">
              Games
            </button>
            {selectedGame && (
              <>
                <span className="shrink-0">/</span>
                <span className="text-slate-100 truncate">{selectedGame.name}</span>
              </>
            )}
            {view === 'deploy' && <><span className="shrink-0">/</span><span className="text-slate-100 shrink-0">Deploy</span></>}
          </nav>

          {/* Secondary nav — full row on sm+; collapses behind the toggle below on mobile */}
          <div className="hidden sm:flex ml-auto items-center gap-2">
            ...
            <button
              onClick={() => { setView('stats'); setSelectedGame(null) }}
              className={`px-3 py-1.5 rounded-lg text-sm border ${view === 'stats' ? 'bg-slate-800 border-slate-600' : 'border-transparent hover:bg-slate-800'}`}
            >
              Stats API
            </button>
            <button
              onClick={() => { setView('monitoring'); setSelectedGame(null) }}
              className={`px-3 py-1.5 rounded-lg text-sm border ${view === 'monitoring' ? 'bg-slate-800 border-slate-600' : 'border-transparent hover:bg-slate-800'}`}
            >
              Monitoring
            </button>
            {/* Steam credentials screen is hidden while DayZ — its only
                consumer — is out of the deployable allowlist. Nothing else
                needs an operator's Steam account, so exposing a password
                field would collect a real credential for a feature that
                cannot currently be used. Restore alongside dayz-kube. */}
            <button
              onClick={logout}
          ...
          {/* Mobile menu toggle — only below sm */}
          <button
            onClick={() => setMobileMenuOpen(v => !v)}
            aria-label={mobileMenuOpen ? 'Close menu' : 'Open menu'}
            aria-expanded={mobileMenuOpen}
            className="sm:hidden ml-auto p-2 rounded-lg border border-slate-700 hover:bg-slate-800 shrink-0"
          >
            {mobileMenuOpen ? (
        ...
        {/* Mobile menu panel */}
        {mobileMenuOpen && (
          <div className="sm:hidden border-t border-slate-800 px-4 py-3 flex flex-col gap-3">
            ...
            <button
              onClick={() => { setView('storage'); setSelectedGame(null) }}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm border ${view === 'storage' ? 'bg-slate-800 border-slate-600' : 'border-slate-800 hover:bg-slate-800'}`}
            >
              Storage
            </button>
            <button
              onClick={() => { setView('stats'); setSelectedGame(null) }}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm border ${view === 'stats' ? 'bg-slate-800 border-slate-600' : 'border-slate-800 hover:bg-slate-800'}`}
            >
              Stats API
            </button>
            <button
              onClick={() => { setView('monitoring'); setSelectedGame(null) }}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm border ${view === 'monitoring' ? 'bg-slate-800 border-slate-600' : 'border-slate-800 hover:bg-slate-800'}`}
            >
              Monitoring
            </button>
            {/* Hidden with the desktop nav entry above — see that comment. */}
            <button
              onClick={logout}
              className="w-full text-left px-3 py-2 rounded-lg text-sm bg-slate-800 hover:bg-slate-700 border border-slate-700"
            >
              Logout
            </button>
          </div>
      ...
      {deploy && (
        <div className="max-w-[92rem] mx-auto px-4 pt-4">
      ...
      <main>
        ...
        {view === 'stats' && <StatsAPI />}
        {view === 'monitoring' && <MonitoringSettings />}
        {/* SteamAccount is not rendered while DayZ is out of the allowlist. */}
