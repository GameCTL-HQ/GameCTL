...
export function buildFactorioYaml(f = defaultFactorioForm) {
  ...
  docs.push({ apiVersion: 'v1', kind: 'PersistentVolume', metadata: { name: pvName, labels }, spec: isLocal ? {
    ...
    persistentVolumeReclaimPolicy: 'Retain', mountOptions: ['nfsvers=4.2'], nfs: { server: f.nfsServer || '10.0.0.100', path: f.dataPvPath || '/mnt/1TBSSD/factorio' }
  } })
  ...
  docs.push({ apiVersion: 'apps/v1', kind: 'Deployment', metadata: { name, namespace: ns, labels, ...mlbAnno }, spec: {
    replicas: 1, selector: { matchLabels: labels }, template: { metadata: { labels }, spec: {
      ...
      initContainers: [ { name: 'rcon-seed', image: f.image || 'factoriotools/factorio:stable', imagePullPolicy: 'Always',
      ...
      containers: [ { name: 'server', image: f.image || 'factoriotools/factorio:stable', imagePullPolicy: 'Always', env,
        ports: [
