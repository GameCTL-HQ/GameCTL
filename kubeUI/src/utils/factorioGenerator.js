...
export const defaultFactorioForm = {
  ...
  gamePort: 34197, // UDP
  // NOTE: there is deliberately no queryPort here. Factorio does not speak the
  // Steam query protocol -- it is not a Steam-distributed game and its headless
  // server has no query listener at all. The entrypoint's command line only has
  // --port and --rcon-port; there is no third flag to pass. A "Steam query
  // (UDP) 27015" field used to exist and opened a Service port and a
  // containerPort at a process that never binds 27015: confirmed on the live
  // pod, which listens on the game port and nothing else. It configured
  // nothing, so it is gone rather than wired up.
