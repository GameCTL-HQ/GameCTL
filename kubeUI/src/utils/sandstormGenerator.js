...
//
// Base image: GameCTL's own sandstorm-kube (from-scratch: Debian + Valve steamcmd;
...
export const defaultSandstormForm = {
  ...
  image: 'ghcr.io/gamectl-hq/sandstorm-kube:latest',
  ...
  docs.push({
    ...
    spec: {
      ...
      template: { metadata: { labels }, spec: {
        ...
        containers: [ {
          name: 'server', image: f.image || 'ghcr.io/gamectl-hq/sandstorm-kube:latest', imagePullPolicy: 'Always', env,
          command: ['/bin/bash', '-c'],
          args: [buildCommand(f)],
          ports: [
