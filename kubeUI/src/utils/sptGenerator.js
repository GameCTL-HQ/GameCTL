...
export const defaultSptForm = {
  ...
  tz: 'UTC',
  // Networking. 6969 = SPT + Fika HTTP/WS (clients connect here). Optional Fika
  // relay/NAT-punch UDP port for dedicated/relay setups.
  httpPort: 6969,
  fikaUdpPort: 22100,
  ...
  docs.push({
    ...
    spec: {
      ...
      template: { metadata: { labels }, spec: {
        ...
        // Install the NARCONet mod-sync SERVER mod onto the volume before the
        // server boots (SPT loads it from user/mods; sync runs over 6969).
        ...(Number(f.modSync ?? 1) === 1 ? { initContainers: [ {
