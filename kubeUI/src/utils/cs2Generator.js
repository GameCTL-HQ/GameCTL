...
// CS2 servers run on GameCTL's own image (ghcr.io/gamectl-hq/cs2-kube,
// repo GameCTL-HQ/CS2-Kube) — built from scratch: Metamod:Source +
// CounterStrikeSharp + a 28-plugin catalog each fetched from its OWN
// upstream release (SharpTimer, GameModeManager, MatchZy, Deathmatch,
// Retakes set, K4-Arenas, GunGame, …) + the kus-derived mode cfg tree
// (vendored configs-only, MIT). The env/volume contract is kus-compatible,
// so this generator's overlay mechanism carried over unchanged.
// (History: previously ran ghcr.io/kus/cs2-modded-server with an
// install_docker.sh sed-patch command override; before that, bolting
// Metamod onto the vanilla joedwards32/cs2 image failed outright.)
//
// The generator emits a small custom_files OVERLAY (mounted at
// /home/custom_files) that tells the server which mode + map to boot into
// and injects GameCTL's own plugins (GameCtlRtv, GameCtlSurfHUD,
// GameCtlDmRounds). A dedicated SteamCMD init container owns the durable
// game install lifecycle on the PVC: first deploy installs once, ordinary
// restarts reuse the persisted install, and an explicit GameCTL update
// request triggers a one-shot validate/update before the next boot. The
// main image then rsyncs its baked addons layer + mode cfg tree onto the
// install, copies our overlay on top, patches gameinfo.gi, and launches
// `cs2 +exec on_boot.cfg`.
...
export const defaultCs2Form = {
  // Deployment/service name AND the per-server data folder: with the Storage
  // Location picker, data lands at <export>/GameCTL/<serverName>. Default to
  // 'cs2-modded' so the path matches the historical CS2 install folder (the
  // ~65G install + surf records persist there) instead of re-downloading to a
  // fresh 'cs2' folder.
  serverName: 'cs2-modded',
  hostname: 'GameCTL CS2',    // in-game server name
  image: 'ghcr.io/gamectl-hq/cs2-kube:latest',
  // One-shot update+validate on next restart. Default off = install once on
  // the persistent volume, then fast restarts reuse the cached install.
  // Turning this on queues a single `app_update 730 validate` before the
  // next boot, then GameCTL clears the request so later restarts stay fast.
  updateOnStart: false,
  ...
  // Surf records website — optional companion Deployment (CS2-Records repo)
  // serving leaderboards/profiles/replays from the GameCtlSurfHUD JSON on
  // the game volume. recordsSiteBrand is the owner name on the site.
  deploySurfRecords: 0,
  recordsSiteBrand: '',
...
export function buildCs2Yaml(f = defaultCs2Form) {
  ...
  const serverCfg = [
    ...
    '// GOTV port. The stock server.cfg pins tv_port 27020, so without this the',
    '// wizard\'s TV port only moved the Service and the LB while GOTV (once',
    '// enabled) still bound 27020 — spectators connecting to a dead port.',
    '// Set here rather than in server.cfg because this file is execd last and',
    '// therefore wins over whatever the mode overlay left behind. GOTV itself',
    '// stays off (tv_enable 0) until an operator turns it on; this only makes',
    '// sure it lands on the right port when they do.',
    `tv_port ${tvPort}`,
  ...
  // an admin belongs to actually grant. admins.json only assigns group
  // membership; without these definitions the groups are empty and admins
  // can't run ANY command (change map/mode, kick, etc.). The kus base image
  // shipped this, but GameCTL's custom_files overlay replaces the configs
  // dir, so we must write it ourselves or it goes missing on a pod recreate.
  // Static + always written (unlike admins.json, this isn't user data).
  const adminGroupsSeed = JSON.stringify({
    '#css/admin': {
      flags: ['@css/reservation', '@css/generic', '@css/kick', '@css/ban', '@css/unban', '@css/vip',
        '@css/slay', '@css/changemap', '@css/cvar', '@css/config', '@css/chat', '@css/vote',
        '@css/password', '@css/rcon', '@css/cheats', '@css/root'],
      immunity: 100,
    },
    '#css/moderator': {
      flags: ['@css/reservation', '@css/generic', '@css/kick', '@css/ban', '@css/unban', '@css/slay',
        '@css/changemap', '@css/cvar', '@css/config', '@css/chat', '@css/vote', '@css/rcon'],
      immunity: 99,
    },
  }, null, 2)
  ...
  const adminGroupsDest = `${OV}/addons/counterstrikesharp/configs/admin_groups.json`
  ...
  writeSteps.push(`fi`)
  // admin_groups.json — always written so the groups admins reference are
  // actually defined (otherwise admins have membership but zero permissions).
  writeSteps.push(`cat > ${adminGroupsDest} <<'GAMECTL_EOF'\n${adminGroupsSeed}\nGAMECTL_EOF`)
  writeSteps.push(`echo "[gamectl] admin_groups.json written (#css/admin, #css/moderator)"`)
  ...
  const installScript = [
    ...
    'INSTALLED="$ROOT/.gamectl-cs2-installed"',
    'LASTREQ="$ROOT/.gamectl-cs2-last-update-request"',
    'REQ="${GAMECTL_UPDATE_REQUEST:-}"',
    'mkdir -p "$ROOT"',
    'MODE=""',
    'if [ ! -f "$INSTALLED" ]; then',
    '  MODE="install"',
    'elif [ -n "$REQ" ] && { [ ! -f "$LASTREQ" ] || [ "$(cat "$LASTREQ" 2>/dev/null)" != "$REQ" ]; }; then',
    '  MODE="validate"',
    'fi',
    'if [ -z "$MODE" ]; then',
    '  echo "[gamectl] cs2 install cache present — skipping steamcmd"',
    '  exit 0',
    'fi',
    'if [ "$MODE" = "install" ]; then',
    '  echo "[gamectl] first CS2 install on persistent volume — running steamcmd app_update"',
    // A failed FIRST install is fatal on purpose: there is no game to fall back
    // to, so starting the server would only crashloop one layer further in.
    '  "$STEAMCMD" +force_install_dir "$ROOT" +login anonymous +app_update 730 +quit',
    '  touch "$INSTALLED"',
    'else',
    '  echo "[gamectl] one-shot CS2 update requested — running steamcmd app_update 730 validate"',
    // A failed UPDATE must not be fatal. The game is already installed and
    // playable, so refusing to start helps nobody. Previously `set -e` aborted
    // here before LASTREQ was written, so the same failing update re-ran on
    // every restart forever -- one stale request cost 119 restarts and ~10h of
    // downtime before anyone noticed the server simply never came up.
    '  if "$STEAMCMD" +force_install_dir "$ROOT" +login anonymous +app_update 730 validate +quit; then',
    '    echo "[gamectl] CS2 update completed"',
    '  else',
    '    echo "[gamectl] WARNING: CS2 update FAILED — starting on the existing install."',
    '    echo "[gamectl] The request is recorded so it will not retry in a loop;"',
    '    echo "[gamectl] trigger another update from GameCTL once the cause is fixed."',
    '  fi',
    'fi',
    // Recorded unconditionally: this marks "we acted on this request", not "the
    // update succeeded". That distinction is what stops the retry loop.
    'if [ -n "$REQ" ]; then',
    '  printf "%s" "$REQ" > "$LASTREQ"',
    'fi',
  ].join('\n')
  ...
  const initContainers = [{
    ...
    // NOT a login shell. As PID 1 a login shell has SHLVL=1, so on exit
    // bash sources ~/.bash_logout, which in Debian-based images runs
    //   [ -x /usr/bin/clear_console ] && /usr/bin/clear_console -q
    // clear_console fails in a container (no console), and because the
    // install script runs under `set -e` — still in effect while
    // .bash_logout executes — that failure REPLACES the script's explicit
    // `exit 0`. The init container then reports exit 1 and the pod sits in
    // Init:CrashLoopBackOff, having logged nothing but its success message.
    // Cost us a downed CS2 server when cm2network/steamcmd:latest picked up
    // a base image carrying that .bash_logout. Nothing here needs a login
    // shell; plain -c avoids the whole class of problem.
    command: ['/bin/bash', '-c'],
    args: [installScript],
    env: [{
      name: 'GAMECTL_UPDATE_REQUEST',
      valueFrom: {
        fieldRef: { fieldPath: `metadata.annotations['${cs2UpdateRequestAnno}']` },
      },
    }],
    volumeMounts: [
  ...
  // The cs2-kube image is configured through these env vars (kus-compatible
  // names — see CS2-Kube's entrypoint.sh). Hostname is NOT an env var —
  // it's set by our gamectl_server.cfg overlay above.
  const env = [
    ...
    // Engine slot cap, not the visible player count. Fixed at 64 for parity
    // with the kus-era setup (its install_docker.sh was sed-patched to
    // -maxplayers 64 regardless of this env): surf/DM fill slots with bots
    // and hit "The game is full" at lower caps. Visible slots stay driven by
    // sv_visiblemaxplayers in the mode overlays.
    { name: 'MAXPLAYERS', value: '64' },
    { name: 'LAN', value: '0' },
    ...
    // Keep the image entrypoint on the fast path; GameCTL's own init
    // container above owns first-install + one-shot update/validate.
    // Presence of this env var keeps the manage screen's generic
    // auto-update toggle wired up through autoUpdateVars.
    { name: 'GAMECTL_VALIDATE', value: '0' },
  ]
  ...
  const depAnnotations = {
    ...(mlbAnno.annotations || {}),
    'gamectl.io/auto-update': (f.updateOnStart === true || f.updateOnStart === 'true') ? 'on' : 'off',
  ...
  docs.push({
    ...
    spec: {
      ...
      template: {
        metadata: { labels },
        spec: {
          // The image's entrypoint drops to the steam user (uid/gid 1000)
          // via setpriv before launching cs2; fsGroup lets that user own
          // the mounted volumes. No privileged pod.
          securityContext: { fsGroup: 1000 },
          initContainers,
          containers: [{
            name: depName,
            image: f.image || 'ghcr.io/gamectl-hq/cs2-kube:latest',
            imagePullPolicy: 'IfNotPresent',
            // No command override — the cs2-kube entrypoint natively covers
            // everything the kus-era sed-patch block bolted on: -maxplayers
            // from MAXPLAYERS, stdbuf line-buffered logs, addons-layer sync,
            // and gameinfo.gi registration every boot. SteamCMD ownership now
            // lives in GameCTL's init container above so persisted installs
            // boot quickly unless GameCTL requested a one-shot update.
            env,
            ports: [
  ...
  // Optional surf-records website (wizard toggle). Standalone read-only
  // viewer for the JSON the GameCtlSurfHUD plugin writes to the game volume
  // (records/attempts/replays at the volume root) — leaderboards, player
  // profiles, replay playback. Repo: GameCTL-HQ/CS2-Records. Mounts the SAME
  // PVC read-only (NFS RWX allows concurrent mounts); no database involved.
  // ClusterIP only — expose it with ProxyCTL / your ingress of choice.
  if (Number(f.deploySurfRecords) === 1) {
    const recName = `${depName}-records`
    // component + companion-of mark this as a side-service of the game, so the
    // Hub nests it UNDER the parent CS2 card instead of listing it as its own
    // "game" instance. (part-of=games is still force-stamped by the wizard; the
    // Hub groups by companion-of, not part-of.)
    const recLabels = {
      app: recName, game: 'cs2',
      'app.kubernetes.io/part-of': 'games',
      'app.kubernetes.io/managed-by': 'gamectl',
      'app.kubernetes.io/component': 'records',
      'gamectl.io/companion-of': depName,
    }
    docs.push({
      apiVersion: 'apps/v1', kind: 'Deployment',
      metadata: { name: recName, namespace: ns, labels: recLabels },
      spec: {
        replicas: 1,
        selector: { matchLabels: { app: recName } },
        template: {
          metadata: { labels: recLabels },
          spec: {
            containers: [{
              name: recName,
              image: 'ghcr.io/gamectl-hq/cs2-records:latest',
              imagePullPolicy: 'Always',
              env: [
                { name: 'GAMECTL_RECORDS_PATH', value: '/data/gamectl_surf_records.json' },
                { name: 'GAMECTL_ATTEMPTS_PATH', value: '/data/gamectl_surf_attempts.jsonl' },
                { name: 'GAMECTL_REPLAYS_DIR', value: '/data/gamectl_surf_replays' },
                // Owner name in the site title/header/footer:
                // "<brand> CS2 Surf Records".
                { name: 'SITE_BRAND', value: f.recordsSiteBrand || f.hostname || 'GameCTL' },
            ...
            volumes: [
              { name: 'data', persistentVolumeClaim: { claimName: pvcName, readOnly: true } },
            ],
          },
        },
      },
    })
    // Mount declared at container level too — readOnly on both layers.
    docs[docs.length - 1].spec.template.spec.containers[0].volumeMounts = [
    ...
    docs.push({
      ...
      metadata: { name: recName, namespace: ns, labels: recLabels },
      spec: {
        type: 'ClusterIP', selector: { app: recName },
