...
export const defaultCs2Form = {
  ...
  // then runs app_update). Turning this on additionally removes the appmanifest
  // and does a full from-scratch `validate` — slower (re-hashes ~65G) but it's
  // the reliable cure when an update wedges at SteamCMD state 0x6 and the server
  // is stuck on a stale build ("client out of date"). Default off = fast. Drives
  // GAMECTL_VALIDATE; flippable via the manage screen's Auto-update toggle.
  updateOnStart: false,
        ...
        spec: {
          ...
          containers: [{
            ...
            args: [
              ...
              // full, from-scratch reconcile; default 0 = fast plain update.
              // Clearing staging + pre-warming steamcmd (above) is not always
              // enough: once an install is wedged at StateFlags 6 (seen on
              // NFS-backed installs after a Valve build bump), steamcmd keeps
              // running the *incremental* update-job, which re-fails to 0x6 and
              // never applies the new build — so the server stays on the stale
              // build and clients get "client out of date", surviving restarts.
              // Removing the appmanifest is what actually clears it (verified
              // on-cluster 2026-07-12): steamcmd then does a `validate` against
              // the current depot and pulls the latest build. Tied to the force
              // toggle so normal starts stay fast (a full re-verify re-hashes
              // ~65G) and the reset only runs when the operator asks for it.
              "if [ \"${GAMECTL_VALIDATE:-0}\" = \"1\" ]; then " +
              "sudo rm -f /home/steam/cs2/steamapps/appmanifest_730.acf 2>/dev/null || true; " +
              "sudo rm -rf /home/steam/cs2/steamapps/downloading/730 /home/steam/cs2/steamapps/temp/730 2>/dev/null || true; " +
              "grep -q '+app_update 730 validate' /home/cs2-modded-server/install_docker.sh || " +
