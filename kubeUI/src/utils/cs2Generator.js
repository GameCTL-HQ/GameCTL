...
// GameCtlDmRounds). On boot the image's entrypoint: installs/updates CS2
// on the volume (first boot / auto-update toggle only), rsyncs its baked
// addons layer + mode cfg tree onto the install, copies our overlay on
// top, patches gameinfo.gi, and launches `cs2 +exec on_boot.cfg`.
...
export const defaultCs2Form = {
  ...
  // Update+validate on start. Default off = the entrypoint starts the
  // existing install without ever running steamcmd (fast). Turning this on
  // makes the next (re)start remove the appmanifest and run a full
  // `app_update 730 validate` — slower (re-hashes ~65G) but it both pulls
  // the latest build and is the reliable cure when an update wedges at
  // SteamCMD state 0x6 ("client out of date"). Drives GAMECTL_VALIDATE;
  // flippable via the manage screen's Auto-update toggle.
  updateOnStart: false,
      ...
      template: {
        ...
        spec: {
          ...
          containers: [{
            ...
            // from MAXPLAYERS, stdbuf line-buffered logs, boot-time steamcmd
            // only on first install or GAMECTL_VALIDATE=1 (which also removes
            // the appmanifest to clear a StateFlags-6 wedge — verified
            // on-cluster 2026-07-12), and the addons layer + gameinfo.gi
            // registration every boot.
            env,
            ports: [
