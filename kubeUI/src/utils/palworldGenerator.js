...
export const defaultPalworldForm = {
  ...
  // default: PalWorldSettings.ini ships PublicIP="", so the server works out
  // its own address and advertises that to the community list. What it works
  // out is how it looks from outside — the cluster's WAN — not the droplet
  // the public ports live on.
  //
  // Bandwidth: the registration heartbeats are small, but being LISTED is
  // not. Every client browsing the community list queries this server, and a
  // public query port draws constant internet-wide scanning on top. All of
  // it crosses the droplet. Turning egress off keeps the server unlisted, so
