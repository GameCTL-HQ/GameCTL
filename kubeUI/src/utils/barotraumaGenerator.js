...
export const defaultBarotraumaForm = {
  ...
  // default because Barotrauma's master server lists a server at the address
  // its registration arrived FROM; without this the master records the
  // cluster's own WAN while the public ports live on the droplet, so the
  // listing points at an address nothing is listening on.
  //
  // Bandwidth: the registration heartbeats are small, but being LISTED is
  // not. Every client browsing the server list queries this server, and a
  // public query port draws constant internet-wide scanning on top of that.
  // All of it crosses the droplet. Turning egress off keeps the server out
  // of the list entirely, so that inbound query load never arrives — that,
  // not the heartbeats, is the real reason to switch it off.
  publishEgress: 1,
  lbIP: '10.0.0.197',
}
...
export function buildBarotraumaYaml(f = defaultBarotraumaForm) {
  ...
  docs.push({
    ...
    metadata: {
      ...
      annotations: {
        ...(mlbAnno.annotations || {}),
        // Omitted entirely when the operator turns egress off, so GameCTL
        // publishes the entry inbound-only and injects no WireGuard sidecar.
        ...(Number(f.publishEgress ?? 1) === 1 ? { 'gamectl.io/publish-mode': 'egress' } : {}),
      },
    },
    spec: {
