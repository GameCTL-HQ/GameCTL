...
// Left 4 Dead 2 — GameCTL's own from-scratch srcds image (Steam app 222860,
// anonymous). Fleet NFS-install model: ~11GB installs to the volume on first
...
export const defaultLeft4dead2Form = {
  ...
  serverName: 'left4dead2',
  image: 'ghcr.io/gamectl-hq/left4dead2-kube:latest',
  storageMode: 'remote',
  nfsServer: '10.0.0.100',
  dataPvPath: '/mnt/1TBSSD/GameCTL/left4dead2',
  dataStorage: '20Gi',
  localDataPath: '/mnt/1TBSSD/GameCTL/left4dead2',
  hostname: 'GameCTL L4D2',
  serverPassword: '',
  rconPassword: '',
  maxPlayers: 8,
...
export function buildLeft4dead2Yaml(f = defaultLeft4dead2Form) {
  ...
  const name = f.serverName || 'left4dead2'
  const labels = { app: name, game: 'left4dead2', 'app.kubernetes.io/part-of': 'games', 'app.kubernetes.io/managed-by': 'gamectl' }
  ...
  docs.push({
    ...
    spec: isLocal
      ? { capacity: { storage: f.dataStorage || '20Gi' }, accessModes: ['ReadWriteOnce'],
          ...
          hostPath: { path: f.localDataPath || '/mnt/1TBSSD/GameCTL/left4dead2', type: 'DirectoryOrCreate' } }
      : { capacity: { storage: f.dataStorage || '20Gi' }, accessModes: ['ReadWriteMany'],
          ...
          nfs: { server: f.nfsServer || '10.0.0.100', path: f.dataPvPath || '/mnt/1TBSSD/GameCTL/left4dead2' } },
  })
  ...
  const env = [
    { name: 'SERVER_NAME', value: f.hostname || 'GameCTL L4D2' },
    ...
    { name: 'START_MAP', value: f.startMap || 'c2m1_highway' },
    ...
    spec: {
      ...
      template: {
        ...
        spec: {
          ...
          containers: [{
            name: 'server',
            image: f.image || 'ghcr.io/gamectl-hq/left4dead2-kube:latest',
