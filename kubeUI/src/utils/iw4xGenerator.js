...
// Base MW2 multiplayer maps — exactly the 16 that ship in zone/<language> of a
// stock install (verified against a real one). DLC maps (Stimulus/Resurgence,
// e.g. Bailout/mp_complex) are deliberately absent: they only exist if the
// operator's copy has the packs, and offering a map the server cannot load
// just produces a failed map change with no explanation.
...
export const defaultIw4xForm = {
  ...
  // Map rotation — defaults to every base map, and deliberately NOT empty.
  // IW4x parses sv_mapRotation exactly once, behind a static guard, at the
  // first match end (MapRotation::LoadMapRotation). If it is empty at that
  // moment the rotation latches empty for the entire process lifetime and
  // sv_nextMap becomes the sentinel "map_restart" — the server then sits on
  // its boot map forever, and setting the dvar later over RCON can never fix
  // it. So "no rotation" is not a harmless default; it is a trap.
  mapRotation: 'mp_afghan, mp_rust, mp_terminal, mp_highrise, mp_favela, mp_derail, mp_estate, mp_quarry, mp_underpass, mp_subbase, mp_rundown, mp_boneyard, mp_invasion, mp_checkpoint, mp_brecourt, mp_nightshift',
  // 0 = no spectating, 1 = own team only (MW2's default), 2 = free — spectate
  // anyone. Read from the live server: scr_game_spectatetype.
  spectateMode: 2,
  extraCfg: '',
  manageConfig: 1,
  ...
  gamePort: 28960,
  // Route the server's OUTBOUND traffic through ProxyCTL's droplet. On by
  // default: IW4x heartbeats to the dpmaster, which lists the server at the
  // address the heartbeat came FROM. Without this the master records the
  // cluster's WAN while the public port lives on the droplet, so the server
  // shows in the browser at an address nobody can reach — or not at all.
  //
  // Bandwidth: the registration heartbeats are small, but being LISTED is
  // not. Every client browsing the list queries this server, and a public
  // query port draws constant internet-wide scanning on top. All of it
  // crosses the droplet. Turning egress off keeps the server unlisted, so
  // that inbound query load never arrives — the real reason to switch off.
  publishEgress: 1,
  lbIP: '',
}

// sv_mapRotation drives what happens at the end of every match. Each entry
// carries its own gametype, so a rotation can mix modes; the server's
// configured gametype applies to every map unless the operator wrote an
// explicit "map:gametype" pair. Shared by the config writer and the init
// container that seeds the rotation file, so the two can never disagree.
function rotationEntries(f) {
  return String(f.mapRotation || '')
    .split(/[\s,]+/)
    .map((e) => e.trim())
    .filter(Boolean)
    .map((entry) => {
      const [map, gt] = entry.split(':')
      return `gametype ${(gt || f.gametype || 'war').trim()} map ${map.trim()}`
    })
}
...
function buildServerCfg(f) {
  ...
  const lines = [
    ...
    `set scr_game_spectatetype "${[0, 1, 2].includes(Number(f.spectateMode)) ? Number(f.spectateMode) : 2}"`,
    'set sv_punkbuster "0"',
    'set sv_cheats "0"',
  ]
  ...
  const rotation = rotationEntries(f)
  if (rotation.length) {
    lines.push(
      '// Map rotation — parsed ONCE at the first match end, so it must be here',
      '// in the config rather than set later over RCON. Single spaces only:',
      '// the engine splits on one space and a doubled one truncates the list.',
      `set sv_mapRotation "${rotation.join(' ')}"`,
      '// …then let the manage screen override it. Executed after the line',
      '// above, so a rotation edited in the panel survives a restart instead of',
      '// being silently reverted to whatever the wizard said at deploy time.',
      'exec gamectl-rotation.cfg',
    )
  }
...
export function buildIw4xYaml(f = defaultIw4xForm) {
  ...
  // Applied after boot over RCON by the image, because the gametype overwrites
  // scr_* dvars set in server.cfg when it initialises — verified live: the
  // config says 2, the server comes up reporting 1. The config line stays as
  // well; it is correct for anyone reading the file, and harmless.
  if ([0, 1, 2].includes(Number(f.spectateMode))) {
    env.push({ name: 'IW4X_SPECTATE', value: String(Number(f.spectateMode)) })
  }
  // Bot Warfare's script overwrites these during init, so the image re-applies
  // them once the server is up. server.cfg keeps them too — correct on paper,
  // just not what the running server ends up with.
  if (f.botSystem === 'botwarfare') {
    if (Number(f.botCount || 0) > 0) env.push({ name: 'IW4X_BOT_FILL', value: String(Number(f.botCount)) })
    if (Number(f.botSkill || 0) > 0) env.push({ name: 'IW4X_BOT_SKILL', value: String(Number(f.botSkill)) })
  }
  ...
  const initContainers = manage ? [{
    ...
    args: [[
      ...
      // Rotation file, seeded from the wizard field on FIRST boot only; the
      // manage screen owns it afterwards. Must exist before the first match
      // ends: IW4x parses the rotation exactly once, and an empty one latches
      // rotation off for the whole process.
      'if [ ! -f /iw4x/players/gamectl-rotation.cfg ]; then',
      `  printf '%s\\n' '// GameCTL-managed — IW4X map rotation. Edit from the manage screen.' 'set sv_mapRotation "${rotation.join(' ')}"' > /iw4x/players/gamectl-rotation.cfg`,
      '  echo "gamectl: seeded players/gamectl-rotation.cfg"',
      'fi',
  ...
  docs.push({
    ...
    // Annotation omitted entirely when the operator turns egress off, so
    // GameCTL publishes inbound-only and injects no WireGuard sidecar.
    metadata: { name, namespace: ns, labels,
      annotations: Number(f.publishEgress ?? 1) === 1 ? { 'gamectl.io/publish-mode': 'egress' } : {} },
    spec: {
      replicas: 1,
