...
export function buildProjectzomboidYaml(f = defaultProjectzomboidForm) {
  ...
  // SERVER_PORT/UDP_PORT are what actually move the server. The image writes
  // them into the PZ ini as DefaultPort/UDPPort; without them PZ stays pinned
  // to its built-in 16261/16262 while the Service, the LB and ProxyCTL's DNAT
  // (which preserves the destination port) all follow the wizard's choice.
  // The result is a server listening where nothing is sent, reporting itself
  // perfectly healthy, with no error in any log.
  const env = [
    ...
    { name: 'UDP_PORT', value: String(base + 1) },
