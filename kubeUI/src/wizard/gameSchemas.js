  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        title: 'Storage',
        fields: [
          { name: 'storageMode', label: 'Storage', type: 'select', options: [
            { label: 'NFS (shared)', value: 'remote' },
            { label: 'Local host path', value: 'local' },
          ], hint: 'The CS2 install is ~65G — it downloads on first boot and persists here so restarts are fast.' },
          { name: 'nfsServer', label: 'NFS server', type: 'text',
            showIf: (f) => (f.storageMode || 'remote') === 'remote' },
          { name: 'dataPvPath', label: 'NFS export path', type: 'text',
            showIf: (f) => (f.storageMode || 'remote') === 'remote' },
          { name: 'localDataPath', label: 'Host path', type: 'text',
            showIf: (f) => f.storageMode === 'local' },
      ...
      },
      {
        ...
        fields: [
          { name: 'steamApiKey', label: 'Steam Web API key', type: 'text',
            ...
            hint: 'Required for Workshop maps — surf / 1v1 / aim / custom maps are all Workshop content. Get a free key at steamcommunity.com/dev/apikey.' },
          { name: 'gslt', label: 'GSLT (game server login token)', type: 'text',
            placeholder: 'app-730 token — optional',
            hint: 'Recommended for an internet-facing server: persistent identity + public server-browser listing. Get one at steamcommunity.com/dev/managegameservers (App ID 730).' },
