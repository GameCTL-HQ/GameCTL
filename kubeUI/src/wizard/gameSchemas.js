...
// Shared egress-publish toggle, for the games whose public listing depends
// on the address their OUTBOUND registration came from (Barotrauma's master,
// IW4x's dpmaster, Wreckfest 2's PlayFab, Palworld's community list).
//
// Default ON, and it should stay on for anyone who wants the server listed:
// with it off, the backend records the cluster's own WAN address while the
// public ports live on ProxyCTL's droplet, so the listing points somewhere
// nothing is listening. That failure is completely silent — the server is
// healthy, externally queryable, and simply never appears.
//
// The hint states the bandwidth trade honestly rather than implying the
// toggle is free either way. The cost is mostly INBOUND, not outbound: the
// registration heartbeats are small, but a listed server is continuously
// queried by every client browsing the list and by internet-wide scanners
// hunting public query ports, and all of that crosses the droplet. Turning
// this off keeps the server out of the list entirely, so that query load
// never arrives — which is the real reason an operator might want it off.
const egressPublishField = (what = 'the public server list') => ({
  name: 'publishEgress',
  label: 'Route outbound traffic through the proxy (needed to appear in server lists)',
  type: 'select',
  options: [
    { label: 'On — server can appear in ' + what, value: 1 },
    { label: 'Off — direct connect only, not listed', value: 0 },
  ],
  hint: 'On: the server registers over the WireGuard tunnel to your proxy droplet, so the '
    + 'address recorded in ' + what + ' is the one players can actually reach. '
    + 'Expect real, continuous bandwidth on the droplet as a result — being listed means '
    + 'every client browsing ' + what + ' queries your server, and public query ports also '
    + 'attract constant internet-wide scanning. That inbound query traffic is typically far '
    + 'larger than the registration heartbeats themselves, and all of it crosses the droplet. '
    + 'Off: the server never registers, so it appears in no public browser and receives none '
    + 'of that query load — players connect by address instead. Gameplay traffic for players '
    + 'who do connect crosses the droplet either way.',
})
...
export const games = [
    ...
    steps: [
      ...
      { id: 'network', title: 'Networking', fields: [
        { name: 'httpPort', label: 'SPT/Fika port (TCP — clients connect here)', type: 'number', required: true,
          hint: 'The only port clients use. Fika relays peer traffic over it too — there is no separate relay port to open.' },
        poolField(),
        lbIPField(),
      ]},
      { id: 'review', title: 'Review', fields: [] },
  ...
  {
    id: 'iw4x',
    // Announces itself to its own public server browser, so the manage
    // screen offers the server-listing toggle. NOT the Stats API export
    // switch beside it — different system entirely.
    supportsServerListing: true,
    ...
    steps: [
      ...
      { id: 'network', title: 'Networking', fields: [
        ...
        egressPublishField('the IW4x master server browser'),
        poolField(),
        lbIPField(),
      ]},
      { id: 'review', title: 'Review', fields: [] },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'network', title: 'Networking', fields: [
        { name: 'gamePort', label: 'Game port (UDP)', type: 'number', required: true },
        // No Steam query port: Factorio has no query listener at all (it is not
        // a Steam-distributed game). The field that used to sit here opened a
        // Service port at a process that never binds it.
    ...
    steps: [
      ...
      resourceStep(RES.wreckfest2),
      {
        ...
        fields: [
          ...
          egressPublishField('the Wreckfest 2 lobby browser'),
        ],
      },
      { id: 'review', title: 'Review', fields: [] },
    ],
    ...
    steps: [
      ...
      resourceStep(RES.satisfactory),
      {
        ...
        fields: [
          poolField(),
          lbIPField(),
          { name: 'serverGamePort', label: 'Game port (TCP + UDP)', type: 'number', required: true,
            hint: 'Satisfactory 1.0 carries everything on this one port. The separate beacon (15000) and query (15777) ports were removed from the game in Update 8 and no longer exist.' },
  ...
  },
  {
    ...
    // capsule art rather than the header: same landscape shape, so the hub
    // grid is unaffected, but a different image.
    cover: 'https://cdn.cloudflare.steamstatic.com/steam/apps/1623730/capsule_616x353.jpg',
    ...
    steps: [
      ...
      { id: 'network', title: 'Networking',
        ...
        egressPublishField('the Palworld community server list'),
        poolField(),
        lbIPField(),
      ]},
      { id: 'review', title: 'Review', fields: [] },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'network', title: 'Networking', fields: [
        { name: 'serverPort', label: 'Game port (UDP)', type: 'number', required: true },
        egressPublishField('the Barotrauma server browser'),
        poolField(),
        lbIPField(),
      ]},
      { id: 'review', title: 'Review', fields: [] },
...
const SUPPORTED_GAME_IDS = new Set([
  ...
  'corekeeper',
  'cs2',
  // 'dayz' — intentionally NOT deployable. DayZ still runs on a third-party
  // SteamCMD base image and is the only game that needs the operator's own
  // Steam account (app 223350 is entitlement-gated, so anonymous login gets
  // an empty install). Until the first-party dayz-kube image lands and the
  // whole path is verified end to end, the game stays out of the allowlist
  // and the Steam credentials screen stays hidden with it — nobody should be
  // asked for their Steam password to drive something unfinished.
  'factorio',
  'iw4x',
  'left4dead2',
  'leftfordead',
