      ...
      },
      {
        ...
        fields: [
          ...
          { name: 'gameModeChoice', label: 'Game mode', type: 'select', options: [
            ...
            { label: 'Surf (timer + records)', value: 'surf' },
            { label: 'Custom (set raw game_type / game_mode)', value: 'custom' },
          ], hint: 'Bots are auto-filled and actually play this mode (server is kept un-hibernated and bots join immediately). Team modes balance to ~5v5 (Wingman 2v2); Deathmatch/Arms Race fill to the player cap. Surf installs the SharpTimer plugin stack and runs bot-free — pick a surf workshop map below.' },
          // Surf needs a Workshop map (all surf maps are Workshop content).
          // workshopMap wins over workshopCollection if both are filled.
          { name: 'workshopMap', label: 'Workshop map ID', type: 'text',
            placeholder: 'e.g. 3070290869 (surf_utopia, surf_kitsune, …)',
            hint: 'Steam Workshop numeric ID of a single map. When set, the server boots this map and the Start map dropdown above is ignored. Required for Surf.',
            showIf: (f) => f.gameModeChoice === 'surf' },
          { name: 'workshopCollection', label: 'Workshop collection ID (optional)', type: 'text',
            placeholder: 'e.g. a surf-map collection ID',
            hint: 'Alternative to a single map: a Workshop collection ID. The server downloads the whole collection; switch maps in-game via the live panel. Leave the map ID blank to use a collection.',
            showIf: (f) => f.gameModeChoice === 'surf' },
