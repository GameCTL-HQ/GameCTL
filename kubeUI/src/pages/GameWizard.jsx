...
function portConflict(ports, port, protocol, lbIP, self) {
  ...
  const describe = (p) => p.source === 'proxyctl'
    ? `${p.service} (${p.portName || 'published'})`
    : `${p.game || p.service} (${p.namespace}${p.portName ? `/${p.portName}` : ''})`

  // A public port ProxyCTL already routes is the one collision that cannot be
  // shared at all: one public port reaches exactly one target. Publishing
  // would silently remap to a free number, leaving what you deployed here and
  // what players actually connect to permanently out of step — so it outranks
  // the LoadBalancer check and is never overridable.
  const published = owners.filter((p) => p.source === 'proxyctl')
  if (published.length) {
    return {
      level: 'error',
      message: `${port}/${proto} is already published through ProxyCTL by ${published.map(describe).join(', ')}. A public port routes to exactly one server — pick another so the deployed port and the public port stay the same number.`,
    }
  }
            ...
            <div className="mt-4 rounded-lg border border-sky-800 bg-sky-950/30 p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-sky-300">
                  {step.seedTitle || 'Seed the Fika mods (run after deploy, once per version)'}
                </span>
                <button
                  type="button"
              ...
              <p className="text-xs text-slate-400 mb-2">
                {step.seedNote || 'The image ships no mods — download the matched Fika pair, then run these (paths are derived from the Storage Location above).'}
              </p>
