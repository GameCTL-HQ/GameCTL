...
function CardMiniGraph({ ns, name }) {
  ...
  const latencyHistory = probeHistory.filter(s => s.reachable && s.latencyMs > 0)
    ...
    <div className="mt-2 pt-2 border-t border-slate-800 space-y-1.5">
      {probeHistory.length >= 2 && (
        <div>
          ...
          {/* up/down heartbeat, not a latency line — meaningful for every
              game including UDP-only ones with no latency probe (which
              previously showed a misleading flat 0ms "uptime" graph). */}
          <HeartbeatStrip samples={probeHistory.map(s => ({ t: s.t, up: s.reachable }))} hours={1} maxBars={40} compact />
        </div>
      )}
      {/* Latency only when the game actually reports it (has a protocol
          probe) — omitted entirely otherwise, no 0ms placeholder. */}
      {latencyHistory.length >= 2 && (
        <div>
          <p className="text-[9px] text-slate-500 leading-none mb-0.5">Latency</p>
          <Sparkline history={latencyHistory.map(s => ({ t: s.t, latencyMs: s.latencyMs }))}
...
export default function GameHub({ onSelectGame }) {
  ...
  const [metricsInstallBusy, setMetricsInstallBusy] = useState(false)
  const [metricsInstallMsg, setMetricsInstallMsg] = useState('')
  ...
  useEffect(() => {
    if (metrics.available) {
      setMetricsServer(null)
      setMetricsInstallMsg('')
      return
    }
    let stop = false
    api.get('/cluster/metrics-server')
      .then(r => { if (!stop) setMetricsServer(r.data) })
      .catch(() => {})
    return () => { stop = true }
  }, [metrics.available])
  ...
  const installMetricsServer = async () => {
    ...
    setMetricsInstallBusy(true)
    setMetricsInstallMsg('')
    try {
      const { data } = await api.post('/cluster/metrics-server/install', { confirm: true })
      setMetricsServer(data.status || null)
      setMetricsInstallMsg('metrics-server install applied. Metrics usually start appearing within about 30 seconds.')
    } catch (e) {
      setMetricsInstallMsg('Install failed: ' + (e.response?.data?.detail || e.message))
    } finally {
      setMetricsInstallBusy(false)
    }
  }
  ...
  return (
    <div className="max-w-[92rem] mx-auto px-4 py-8 space-y-10">
      ...
      {!metrics.available && metricsServer && (
        <section className="rounded-xl border border-sky-900 bg-sky-950/30 p-4">
          <p className="text-sm font-semibold text-sky-300 mb-1">
            CPU and RAM graphs need `metrics-server`
          </p>
          <p className="text-xs text-slate-300">
            {metricsServer.available
              ? 'metrics-server is installed and responding. Wait about 30 seconds for the first sample to land.'
              : metricsServer.installed
                ? `metrics-server exists but is not healthy yet${metricsServer.detail ? `: ${metricsServer.detail}` : ''}`
                : 'This cluster does not have metrics-server installed, so game entries cannot show CPU and RAM usage.'}
          </p>
          {metricsServer.offerInstall && (
            <button
              onClick={installMetricsServer}
              disabled={metricsInstallBusy}
              className="mt-3 rounded bg-emerald-600 px-3 py-1.5 text-xs font-medium text-white disabled:opacity-60"
            >
              {metricsInstallBusy ? 'Installing…' : 'Install metrics-server'}
            </button>
          )}
          {metricsInstallMsg && <p className="mt-2 text-[11px] text-slate-400">{metricsInstallMsg}</p>}
        </section>
      )}
