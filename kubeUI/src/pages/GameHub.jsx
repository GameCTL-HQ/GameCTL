...
import { deployableGames, unsupportedGames } from '../wizard/gameSchemas'
...
// How much history the hub cards summarize. Rendered onto each strip's
// label ("Uptime · 1h") so the numbers aren't ambiguous — the manage screen
// has a duration picker, the cards are fixed, and without saying so a card
// reading "RAM · 92%" gives no clue what period that covers.
const CARD_WINDOW_HOURS = 1
const CARD_WINDOW_LABEL = ` · ${CARD_WINDOW_HOURS}h`
...
function CardMiniGraph({ ns, name }) {
  ...
  const cutoff = Date.now() / 1000 - CARD_WINDOW_HOURS * 3600
  ...
  return (
    <div className="mt-2 pt-2 border-t border-slate-800 space-y-1.5">
      {probeHistory.length >= 2 && (
        <div>
          <p className="text-[9px] text-slate-500 leading-none mb-0.5">Uptime{CARD_WINDOW_LABEL}</p>
          ...
          <HeartbeatStrip samples={probeHistory.map(s => ({ t: s.t, up: s.reachable }))} hours={CARD_WINDOW_HOURS} maxBars={40} compact />
        </div>
      )}
      ...
      {resourceHistory.length >= 2 && (
        <>
          <div>
            <p className="text-[9px] text-slate-500 leading-none mb-0.5">CPU{cpuPct !== null ? ` · ${cpuPct}%` : ''}{CARD_WINDOW_LABEL}</p>
          ...
          <div>
            <p className="text-[9px] text-slate-500 leading-none mb-0.5">RAM{memPct !== null ? ` · ${memPct}%` : ''}{CARD_WINDOW_LABEL}</p>
...
export default function GameHub({ onSelectGame }) {
  ...
  // Deployable is the explicit allowlist in gameSchemas, not "everything in
  // the catalog minus a flag" — a generator that hasn't been verified against
  // a cluster must not reach the picker just by existing in the repo.
  const available = deployableGames
  const comingSoon = unsupportedGames.filter(matches)
  ...
  return (
    <div className="max-w-[92rem] mx-auto px-4 py-8 space-y-10">
      ...
      {!metrics.available && <MetricsServerNotice />}

      {/* Only renders when instances predating the Recreate fix exist. */}
      <RestartStrategyNotice />
