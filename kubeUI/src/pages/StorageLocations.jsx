              ...
              <div key={i} className="rounded-lg border border-slate-800 bg-slate-900/40 p-3 space-y-2">
                ...
                <div className="flex items-center gap-2 pt-1">
                  <button
                    ...
                    className="px-3 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-medium disabled:opacity-50"
                  ...
                  {probe?.result && <ProbeResult result={probe.result} />}
                </div>
              </div>
            )
          })}
...
// ProbeResult renders the inline pass/fail strip for a Test click. Green
// when the probe pod completed every stage; red with stage label, message,
// hint and (collapsible) raw details otherwise. The hint text comes from
// the backend's classifier so the wording stays in sync with what the
// probe actually checked.
function ProbeResult({ result }) {
  const { ok, stage, message, hint, durationMs, details } = result || {}
  if (ok) {
    return (
