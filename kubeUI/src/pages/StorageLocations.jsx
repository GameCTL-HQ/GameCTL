...
import { cleanExport, cleanSegment, joinPath } from '../utils/paths'
...
export default function StorageLocations({ firstRun = false, onSkip, onContinue } = {}) {
  ...
  const normalizeRow = (row) => ({
    ...row,
    exportPath: cleanExport(row.exportPath),
    folderSuffix: cleanSegment(row.folderSuffix || ''),
  })
  ...
  const save = async () => {
    ...
    try {
      ...
      const { data } = await api.put('/storage/locations', { locations: clean })
      const next = data?.locations || clean
      setRows(next)
              ...
              <div key={i} className="rounded-lg border border-slate-800 bg-slate-900/40 p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <input className={`${inp} ${local ? 'col-span-2' : ''}`}
                    ...
                    onBlur={e => update(i, 'exportPath', cleanExport(e.target.value))} />
                </div>
                <div className="grid grid-cols-2 gap-2 items-center">
                  <input className={inp} placeholder="folder suffix (optional)"
                    ...
                    onBlur={e => update(i, 'folderSuffix', cleanSegment(e.target.value))} />
                  <span className="text-xs text-slate-500 min-w-0">
                    data → <code className="text-slate-300 break-all">{joinPath(row.exportPath || '<path>', folder)}/&lt;server&gt;</code>
                  </span>
                </div>
