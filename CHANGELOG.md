...
## [v0.0.50-beta] - 2026-08-02

> Deleting a server now cleans up everything it created. Previously a deleted server could leave public ports open on your proxy, forwarding to something that no longer existed.

### Fixed

- **Deleting a server no longer leaves public ports open** — A server listed in its in-game browser holds a tunnel identity on your proxy, and that identity dies with the server. If the "also remove public access" box was unticked, the proxy entry stayed behind: its public ports remained open and forwarded to a server that had been deleted, and a later background sync could even recreate the deleted server's tunnel keys. Nothing reported an error at any point. Listed servers are now always unpublished on delete, whatever that box says, and the delete dialog tells you so. For servers that only take direct connections the box still works as before, so deleting and redeploying under the same name keeps its ports and DNS record.

- **Deleting a server removes its tunnel keys** — The cleanup covered the server, its network services and its storage claims, but never the tunnel keys, so those outlived the server that owned them. They are now always removed.

- **Storage location is shown correctly again for tunnelled servers** — A server whose data lives on a storage claim could display its location as `/lib/modules (local)` instead of its actual share. The tunnel adds a system folder to the server, and the storage display picked that up when it could not find an obvious data folder. Affected Palworld and Barotrauma; display only, no data was moved or touched.

### Changed

- **The server-browser setting is easier to read** — It now states the two outcomes plainly, with **ON** and **OFF** called out, instead of a paragraph you had to read through to find the trade-off.

## [v0.0.49-beta] - 2026-08-02

### Fixed

- **The server-browser switch no longer appears seconds after the page** — On games that support it, the "List in the in-game server browser" switch was waiting on a call that contacts your proxy for entry, domain and droplet details before it could answer. The rest of the settings panel drew immediately and the switch arrived late, by which point you would reasonably have concluded the option wasn't there. It now comes from data the panel already loads and draws with everything else.

## [v0.0.48-beta] - 2026-08-02

> The public server-browser listing can now be switched on or off per server from its manage screen, not just at deploy time.

### Added

- **Server-browser listing is now a switch on each server's page** — Barotrauma, IW4x, Wreckfest 2 and Palworld each get a "List in the in-game server browser" switch, so it can be changed on a running server instead of only when first deploying. It appears only for games that actually register with a browser. Turning it on routes the server's outbound traffic through your proxy so the address its browser records is the one players can reach; turning it off stops the server registering at all. Either way the game restarts once. Leaving it on costs ongoing proxy bandwidth — a listed server is queried continuously by everyone browsing that list, and public query ports attract constant automated scanning.

### Changed

- **"Advertise publicly" is renamed to say what it actually does** — It is now **"Export this server's stats via the GameCTL API"**. The old name sounded like it advertised your server to players, which it never did: it only decides whether the server appears in `GET /api/stats/servers` so your own site or dashboard can read its status and player count. Sitting next to the new server-browser switch, that was an easy and consequential thing to mix up. The setting itself is unchanged.

### Fixed

- **Turning a listing off no longer leaves the proxy half-configured** — Switching it off removed the server's tunnel while the proxy entry still believed the tunnel was there, so the proxy kept routing for something that no longer existed and the server became unreachable from outside. Found by exercising the switch against a real server rather than assuming it worked.

## [v0.0.47-beta] - 2026-08-02

> The proxy tunnel now starts before the game does. Games published through ProxyCTL could previously send their first outbound traffic before the tunnel was ready, which is how a server ends up listed at an address nobody can reach.

### Fixed

- **The proxy tunnel starts before the game** — For games published in egress mode, the WireGuard tunnel ran as an ordinary container alongside the server, and Kubernetes does not wait for one container before starting another. For the moment between the game starting and the tunnel finishing setup, the server's traffic left over your own internet connection instead of the proxy. Games that announce themselves to a public server browser do that announcing the instant they start, so the browser could record the wrong address — leaving a server that is running perfectly, answers queries, and still cannot be joined from the list, with no error anywhere. The tunnel is now a startup dependency: the server does not begin until it is up. Existing published games pick this up the next time they are published.

- **Publishing no longer silently fails to update a game** — A published game whose tunnel setup needed rewriting could be rejected by Kubernetes while the publish still reported success, so the change never landed and nothing said so.

### Added

- **Server-list publishing is now a per-game choice** — Barotrauma, IW4x, Wreckfest 2 and Palworld have a setting for whether the server announces itself to its public server browser. It stays **on** by default, because turning it off means the server will not appear in any list. Left on, expect ongoing bandwidth through your proxy: a listed server is queried continuously by everyone browsing that list, and public query ports also attract constant automated scanning. Turned off, none of that traffic arrives and players connect by address instead.

### Changed

- **Palworld uses different cover art** in the game picker.

## [v0.0.46-beta] - 2026-08-02

> Palworld is now deployable from the wizard. DayZ has been withdrawn from the deployable list, and the Steam credentials screen is hidden with it — nothing else needed it.

### Added

- **Palworld is deployable** — Every piece of Palworld support had shipped except one: it was missing from the allowlist that controls which games the deploy picker offers. The generator, the wizard schema, the first-party image and the docs were all in place, so nothing errored and nothing looked wrong — the game simply appeared greyed out as "coming soon" and its wizard refused to open. It is now selectable and deploys like any other game.

### Removed

- **DayZ is withdrawn until its first-party image is ready** — DayZ was the only game still running on a third-party SteamCMD image, and the only one that required handing GameCTL your personal Steam username and password (its app is entitlement-gated, so an anonymous install produces an empty server). Rather than leave a half-finished path in front of people, DayZ has been taken out of the deployable list and moved to Upcoming.

- **The Steam credentials screen is hidden** — DayZ was its only consumer, so with DayZ withdrawn there is nothing left that needs an operator's Steam account. The screen is removed from the navigation *and* its direct link now falls through to the hub, because hiding only the button would still leave a bookmarked URL asking for a Steam password. Stored credentials are untouched and the screen returns with DayZ.

## [v0.0.45-beta] - 2026-08-02

> An audit of every game found six where the port you choose in the wizard never reached the server. Those servers bind their built-in default instead, so they deploy cleanly, report themselves healthy, and are simply unreachable — with nothing wrong in any log. Only servers moved off their default port were affected. If you changed a port for Minecraft, Project Zomboid or SPT/Fika, redeploy from the wizard to pick up the fix.

### Fixed

- **The port you pick now actually reaches the server** — Choosing a port in the wizard sets the Kubernetes Service, the load balancer address and the proxy forward, all of which follow your choice. For six games the server process itself was never told, so it went on listening on its built-in default while everything in front of it pointed somewhere else. Nothing errors: the container starts, the deployment goes green, the status panel says healthy, and players simply cannot connect. This affected **Minecraft**, **Project Zomboid**, **SPT/Fika**, and the GOTV port on **CS2**; **Barotrauma** and **Necesse** were fixed in earlier releases. Servers left on their default ports were never affected, which is why this went unnoticed for so long.

- **Minecraft no longer overwrites your port on every start** — The Minecraft image did not merely ignore the chosen port; it rewrote `server-port` back to 25565 in `server.properties` each time the container started. Editing the file by hand did not help either, because the next restart undid it. The port is now applied from your choice, and the query port is pinned to match so the two cannot drift apart.

- **BlueMap's port is applied** — BlueMap's web port lives only in its own config file and is not settable by environment variable, so changing it in the wizard moved the Service to a port BlueMap was never listening on. The map returned nothing while BlueMap logged a clean start on 8100 throughout. The chosen port is now written to BlueMap's config before the server starts.

### Removed

- **Ports that never existed are gone from the wizard** — Four fields offered a port for something the game has no listener for, then opened a Service port and a firewall path to it. **Factorio** had a "Steam query" port, though Factorio is not a Steam game and its server has no query listener at all. **Satisfactory** had separate beacon and query ports, which the game removed in Update 8 — since 1.0 everything runs over the single game port, and the running server confirms it binds only that one. **SPT/Fika** had a "relay UDP port" defaulting to 22100, a number matching nothing: Fika's NAT-punch listener is configured elsewhere, defaults to a different port, and ships disabled. These fields did nothing but suggest they worked, so they are removed rather than wired to flags that do not exist. Existing deployments are unaffected — the ports were never in use.

## [v0.0.44-beta] - 2026-08-02

> Palworld shipped in v0.0.43-beta with a health check that could never pass, which would have killed the server part-way through its first download. Anyone who deployed Palworld from v0.0.43-beta should redeploy from the wizard.

### Fixed

- **Palworld no longer kills itself during its first boot** — The generated Deployment carried a startup probe that opened a TCP connection to the server's query port to decide whether the container had finished starting. Both of Palworld's ports are UDP, so that connection could never succeed no matter which port was chosen; every attempt failed, and once the startup budget ran out the container was killed and restarted — turning a perfectly healthy first boot, which downloads about five gigabytes and legitimately takes several minutes, into a crash loop. Kubernetes has no UDP probe and Palworld exposes no TCP port unless RCON is enabled, so the probe is removed rather than pointed somewhere else. This only affected Palworld, and only in v0.0.43-beta.

### Changed

- **Palworld publishes in egress mode** — Palworld's config leaves its public address blank, which makes the server work out its own address and advertise that to the community server list. What it works out is how the server looks from outside, which is the cluster's own connection — not the proxy the public ports actually live on. The listing then points somewhere nothing is listening, and the server appears unjoinable or does not appear at all. Palworld now routes its outbound traffic through the proxy, the same as IW4X and Wreckfest 2, so the address it advertises is the one players can reach.

## [v0.0.43-beta] - 2026-08-02

> Three games could be deployed onto a port nothing was listening on, and it looked like nothing was wrong. When GameCTL moved a server to a free port, the port moved but the game didn't — so the server ran perfectly while being unreachable, with no error anywhere. Also: one failed update could keep a server down indefinitely, and Palworld joins the catalog.

### Fixed

- **A failed update no longer keeps a server down forever** — The CS2 init container ran steamcmd under `set -e` and only wrote its "already handled this request" marker *after* it succeeded. So when an update failed, the marker was never written, the next start saw the same request as new, and it re-ran the identical failing update. Seen in the wild: one forced validate hit a stale Steam appmanifest and latched the server into a crashloop for roughly ten hours and 119 restarts — while the game itself was fully installed and perfectly playable the entire time. None of it was visible in the UI, because a pod's main containers do not start until its init containers finish, so the log pane simply stayed empty. A failed *update* now warns and starts the server on the existing install, and the request is recorded either way — the marker means "we acted on this", not "it worked". A failed *first* install still stops, since there is no game to fall back to.

- **Servers deployed on a reassigned port are reachable again** — GameCTL recommends a different port when the one you picked is already claimed, and both the Service and ProxyCTL's forwarding follow that choice. Barotrauma and Necesse never told the *game* about it, so the game stayed on its default port while everything around it moved. The result is a server that starts cleanly, reports itself healthy, and cannot be connected to by anyone — with nothing in any log to explain it, because from the server's point of view nothing failed. Barotrauma was found running on 27015 behind a forward for 27022. Both now pass the chosen port through to the server itself. Project Zomboid had the same latent problem and is fixed in its image (`Zomboid-Kube`).

- **Barotrauma can be listed in the public server browser** — Three separate faults stacked, and each one hid the next. Its image silently overwrote the query port with the game port on every boot, which is harmless while a server is private but makes it fail to start the moment it is made public. Once that was fixed, the game port and the forwarded port disagreed. Once *that* was fixed, the query port — the one the in-game browser actually probes — was reachable in-cluster but not from outside. Finally, the server's outbound registration left via the cluster's home connection while its public ports lived on the proxy, so the master server advertised an address with nothing listening on it. Barotrauma now publishes in egress mode, the same as IW4X and Wreckfest 2, so the address the master server records is the one players can reach.

### Added

- **Palworld** — Deploy a Palworld dedicated server from the wizard, with the game installing to its volume on first boot rather than into the image, so moving the server between nodes does not re-download it. Both UDP ports are wired up from the start — the game port and the separate Steam query port — because forwarding only the game port is what leaves a server joinable by direct IP but invisible in the browser. Palworld is the heaviest server in the catalog; the default resource profile reflects that, with a ceiling chosen to keep a 16 GB node schedulable for anything else.

## [v0.0.42-beta] - 2026-07-31

> A Factorio server could be bricked permanently by a single storage or network blip. One partial autosave was enough to make it fail on every boot, forever, while a perfectly good save sat next to it. It now cleans up after itself.

### Fixed

- **A partial autosave can no longer brick a Factorio server** — Factorio autosaves by writing <name>.tmp.zip and then renaming it over <name>.zip — atomic only if the rename happens. Interrupt the write (a storage hiccup, a network blip on NFS-backed saves, the pod killed mid-save) and a truncated .tmp.zip is left behind. Because the server starts with --start-server-load-latest it then loads that file, being the newest in saves/, and dies with "Opening zip .../world.tmp.zip failed: I/O error" on every boot from then on — until someone notices and deletes it by hand, with a complete save and five valid autosaves sitting right beside it. GameCTL now moves any partial autosave aside before the server starts. It is moved, not deleted, so the bytes remain if one is ever worth recovering, and load-latest is kept because picking the newest autosave is the right thing to do after a real crash. Existing servers get this on their next deploy from the wizard.
## [v0.0.41-beta] - 2026-07-29

> IW4X gets a real map rotation you can build, save and apply without it being erased or reverted, plus a fix for the Bot Warfare pathfinding that froze servers for 25 seconds at a time. Also fixes a crash that made the Monitoring page unusable.

### Fixed

- **IW4X map changes apply the game type too** — Change map sent the map without the game type, so the server loaded the new map under whatever mode it was already running. The map change is now sent first, then the game type, with a pause between them — sending both at once had the server drop one. Gameplay settings are likewise applied after boot, which is the only point they stick.
- **Bot Warfare no longer freezes IW4X servers for 25 seconds** — The bundled Bot Warfare mod's A* pathfinding could stall the server thread for around 25 seconds at a time — long enough for every player to time out. The image now patches the pathfinder so bot navigation stays inside a sane budget.
- **The Monitoring page white-screened on every visit** — The metrics-server card rendered before its status request had resolved and read a field off a null response, which threw and took the entire page down — Monitoring was unreachable rather than merely missing a panel. It now renders immediately with the install command and fills in the specifics once the status arrives.

### Added

- **IW4X map rotation you can actually manage** — Set the rotation in the deploy wizard, then edit it live from the server's control panel. A saved rotation now survives a restart instead of reverting to the image default, Save / Apply / Restart do what their names say, and changing map by hand no longer gets stomped by the rotation on the next cycle. Applying a rotation also stopped erasing the one you had just built.

### Changed

- **The metrics-server command sits at the top of Monitoring** — Retitled to "Can't see CPU/RAM metrics? Run this" and moved to the top of the page. CPU and RAM graphs are the reason to open Monitoring, so the command that makes them work belongs where you land — not underneath the panels that are empty precisely because it hasn't been run.

### Removed

- **CarX Drift Online and Assetto Corsa** — Both were catalog entries that never reached a working deploy, so they are gone rather than sitting in the picker as dead ends. The IW4X map list was corrected at the same time.
## [v0.0.40-beta] - 2026-07-28

> Two new games: IW4X (Call of Duty: Modern Warfare 2) and DayZ. IW4X runs on GameCTL's own iw4x-kube image — a Wine runtime that downloads nothing, validates the game files you supply, and fails with the missing filename instead of sleeping forever like the abandoned community image it replaces. It comes with a live control panel: roster, map and gametype switching, bot management, kicks, editable bot names, and an admin list for the in-game bot menu. DayZ brings back the shared Steam account page, since its server can't be downloaded anonymously. Plus the DEV BUILD badge works again — it had been silently hiding itself on every dev build since the first release tag.

### Added

- **IW4X (Modern Warfare 2) servers** — Deploy an IW4X dedicated server from the wizard. IW4X is Windows-only, so it runs under Wine in GameCTL's own iw4x-kube image (published at ghcr.io/gamectl-hq/iw4x-kube). You supply the game directory — your own legally owned MW2 install with IW4x installed into it — and the Storage step prints the exact rsync/kubectl commands with your host and path already filled in. The image never downloads game content and never sleeps on failure: a missing iw4x.exe or main/iw_00.iwd is reported by name and the pod exits, so a server that cannot work never reports itself Running and Ready. DLC is optional; base-map-only installs are normal.
- **IW4X live controls on the manage screen** — A panel alongside the CS2 and Wreckfest ones: the live roster with scores, pings and a BOT badge, per-player kick, map and gametype switching, map restart, a say box, and bot fill/spawn. Bot names are editable — per row or as a list — and are written to the file IW4x reads at startup, so the panel tells you a restart is needed rather than pretending the change is live. An admin section grants Bot Warfare's in-game menu (Action Slot 2) to chosen players, with one-click add from the roster so nobody has to retype a 16-character GUID.
- **RCON for idTech3 games** — The RCON console previously spoke only Valve's Source protocol over TCP. IW4X and Quake 3 speak the idTech3 connectionless UDP dialect instead, so their console appeared but every command failed. Both now work, dispatched on the game type, with the console's quick-command buttons filled in for each.
- **DayZ servers** — DayZ's dedicated server (Steam app 223350) cannot be downloaded by anonymous SteamCMD — it needs an account that owns the game. The entry wires that to the shared Steam account below, and a missing login stops the pod with a legible reason instead of silently retrying anonymously and dying minutes later on a missing binary. GameCTL writes its own config file rather than fighting the image's, so turning managed config off hands back an untouched file to edit. Shipped deployable but unverified: booting it needs a DayZ-owning account, which only an operator can supply.
- **Steam account settings page (restored)** — One shared Steam login for games SteamCMD can't fetch anonymously, stored as a write-only Secret in your cluster and referenced by deployments through secretKeyRef — never written into a manifest and never shown again after saving. Removed last May when nothing used it; DayZ uses it again.

### Fixed

- **The DEV BUILD badge stopped hiding on dev builds** — The badge decided whether a build was a release with an unanchored version match. That worked while the repository had no tags, because the version stamp was a bare commit SHA. From the first release tag onward every dev build stamped something like v0.0.39-beta-2-g6b89f15-dirty, whose leading v0.0.39 matched — so the badge vanished exactly when it mattered, and a build running uncommitted code presented itself as a tagged release. It now requires a complete version, so a commit-count suffix or a dirty tree correctly reads as a dev build.

### Changed

- **Steps can title their own file-seeding instructions** — The wizard's copy-paste panel for operator-supplied files had SPT/Fika's wording hardcoded, so any other game needing seeded files got instructions about Fika mods. Games can now supply their own title and explanation; IW4X uses it for MW2 game files.
