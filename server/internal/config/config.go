...
type Config struct {
	...
	// StatsSecret signs GameCTL Stats API tokens — a separate, deliberately
	// distinct key from JWTSecret so a leaked stats token can never pass the
	// admin auth Middleware (it verifies against JWTSecret only). Empty
	// means the Stats API is disabled; it's persisted into the same auth
	// Secret as JWTSecret (key "stats") once an operator enables it via
	// POST /api/stats/enable, so — unlike JWTSecret — it is optional at
	// boot and picked up on the next restart after being persisted.
	StatsSecret []byte
	...
	// PromositeImage is the container image deployed for the optional
	// bundled "promotion site" (docs/STATS_API.md, promosite/). Defaults to
	// the public image; override for a local/private registry.
	PromositeImage string

	// SteamSecretName is the shared Opaque Secret (in SelfNamespace) holding
	// a Steam account's username/password for game servers that can't use
	// anonymous SteamCMD (DayZ, and likely others / Workshop later). One
	// shared account, referenced by deployments via secretKeyRef.
	SteamSecretName string
}
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		PromositeImage:      getenv("GAMECTL_PROMOSITE_IMAGE", "ghcr.io/gamectl-hq/promosite:latest"),
		SteamSecretName:     getenv("GAMECTL_STEAM_SECRET_NAME", "gamectl-steam"),
	}
	...
	// Stats API secret is optional — absent means disabled until the
	// operator enables it from the UI, which persists a generated secret
	// into the auth Secret. If present it's held to the same length floor
	// as the JWT secret (defense in depth against a weak hand-set value).
	if statsSecret := os.Getenv("GAMECTL_STATS_SECRET"); statsSecret != "" {
		if len(statsSecret) < 32 {
			return nil, fmt.Errorf("GAMECTL_STATS_SECRET too short: got %d bytes, need at least 32", len(statsSecret))
		}
		cfg.StatsSecret = []byte(statsSecret)
	}
