...
// clearRollingUpdateForRecreate makes a Deployment switchable from
// RollingUpdate to Recreate.
//
// The API server DEFAULTS spec.strategy.rollingUpdate (maxSurge/
// maxUnavailable) onto every RollingUpdate Deployment. Those defaulted
// subfields are not part of our applied config, so a server-side apply that
// sets type: Recreate merges into `{type: Recreate, rollingUpdate: {...}}`
// and the API server rejects it:
//
//	spec.strategy.rollingUpdate: Forbidden: may not be specified when
//	strategy `type` is 'Recreate'
//
// Verified against a live cluster — an explicit `rollingUpdate: null` in the
// apply body does NOT help, because SSA drops nulls. A JSON merge patch
// does delete on null, so clear the block first and let the apply follow.
//
// No-ops unless we're applying a Recreate Deployment, and a NotFound is the
// normal first-deploy case, not an error.
func clearRollingUpdateForRecreate(ctx context.Context, ri dynamic.ResourceInterface, u *unstructured.Unstructured, dryRun bool) error {
	if u.GetKind() != "Deployment" {
		return nil
	}
	if t, ok, _ := unstructured.NestedString(u.Object, "spec", "strategy", "type"); !ok || t != "Recreate" {
		return nil
	}
	live, err := ri.Get(ctx, u.GetName(), metav1.GetOptions{})
	if err != nil {
		if apierrors.IsNotFound(err) {
			return nil // fresh deploy: nothing to clear
		}
		return err
	}
	if t, ok, _ := unstructured.NestedString(live.Object, "spec", "strategy", "type"); !ok || t == "Recreate" {
		return nil // already Recreate — nothing defaulted to clear
	}
	opts := metav1.PatchOptions{FieldManager: fieldManager}
	if dryRun {
		opts.DryRun = []string{metav1.DryRunAll}
	}
	const clear = `{"spec":{"strategy":{"type":"Recreate","rollingUpdate":null}}}`
	_, err = ri.Patch(ctx, u.GetName(), types.MergePatchType, []byte(clear), opts)
	return err
}
...
func (c *Cluster) Apply(ctx context.Context, docs []map[string]any, dryRun bool, rep PhaseReporter) (ApplyResult, error) {
	...
	for i, raw := range docs {
		...
		if err := clearRollingUpdateForRecreate(ctx, ri, u, dryRun); err != nil {
