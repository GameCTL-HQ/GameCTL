...
func (c *Cluster) RestartInstance(ctx context.Context, ns, name string) error {
	...
	tmplMeta := map[string]any{
		"annotations": map[string]any{
			"gamectl.io/restartedAt": now,
		},
	}
	tmplSpec := map[string]any{}
	rootMeta := map[string]any{}

	// Fold pending desired-state annotations into this rollout's pod
	// template: the auto-update choice and any resources change applied
	// live via in-place resize (SetInstanceResources) — this is what makes
	// those durable across reschedules.
	if len(dep.Spec.Template.Spec.Containers) > 0 {
		cont := dep.Spec.Template.Spec.Containers[0]
		contPatch := map[string]any{"name": cont.Name}
		if choice, ok := dep.Annotations[autoUpdateAnno]; ok {
			if spec := autoUpdateSpecFor(cont.Env); spec != nil {
				if gameKey == "cs2" && spec.Name == "GAMECTL_VALIDATE" {
					contPatch["env"] = []map[string]any{{"name": spec.Name, "value": spec.OffVal}}
					if strings.EqualFold(choice, "on") {
						tmplMeta["annotations"].(map[string]any)[cs2UpdateRequestAnno] = now
						rootMeta["annotations"] = map[string]any{autoUpdateAnno: "off"}
					}
				} else {
					val := spec.OffVal
					if strings.EqualFold(choice, "on") {
					...
					contPatch["env"] = []map[string]any{{"name": spec.Name, "value": val}}
				}
			}
		}
		if raw, ok := dep.Annotations[resourcesAnno]; ok {
			var want InstanceResources
			if json.Unmarshal([]byte(raw), &want) == nil && want.Container == cont.Name {
				if reqs, lims, verr := validateResources(want); verr == nil {
					contPatch["resources"] = map[string]any{
		...
		if len(contPatch) > 1 {
			tmplSpec["containers"] = []map[string]any{contPatch}
		}
	}
	...
	root := map[string]any{"spec": map[string]any{"template": tmpl}}
	if len(rootMeta) > 0 {
...
func (c *Cluster) DeleteInstance(ctx context.Context, ns, name string, opts DeleteOptions, rep PhaseReporter) (DeleteResult, error) {
	...
	{
		// Services by the instance label, so companion Services (e.g.
		// minecraft's separate `<name>-bluemap-service`, which follows
		// neither the `<name>` nor `<name>-service` convention) are swept too
		// — otherwise they orphan and leak their MetalLB LoadBalancer IP.
		// The apply normalizer stamps gamectl.io/instance on every object, so
		// this catches all of an instance's Services in one query. list +
		// delete-by-name (the Role grants both, but not deletecollection).
		ph := rep.BeginPhase("Delete Services", "label "+sel)
		svcs := b.clientset.CoreV1().Services(ns)
		swept := map[string]bool{}
		var serr error
		if list, lerr := svcs.List(ctx, metav1.ListOptions{LabelSelector: sel}); lerr != nil {
			serr = lerr
		} else {
			for _, s := range list.Items {
				if derr := svcs.Delete(ctx, s.Name, metav1.DeleteOptions{}); derr != nil && !apierrors.IsNotFound(derr) {
					serr = derr
				} else if derr == nil {
					swept[s.Name] = true
					out.Deleted = append(out.Deleted, "Service/"+s.Name)
				}
			}
		}
		// Legacy by-name (pre-label instances whose Services lack the
		// gamectl.io/instance label), best-effort; skip names already swept.
		for _, svcName := range []string{name, name + "-service"} {
			if swept[svcName] {
				continue
			}
			if derr := svcs.Delete(ctx, svcName, metav1.DeleteOptions{}); derr == nil {
	...
	// 2b. Egress tunnel keys. The sidecar itself dies with the Deployment,
	//     but its Secret is not covered by any sweep above (Deployments,
	//     Services and PVCs are; Secrets are not), so it outlives the
	//     instance. Deleting it here is unconditional ON PURPOSE: cleanup
	//     used to happen only on the "also unpublish" path, so deleting an
	//     instance with that box unticked orphaned the keys, and a later
	//     reconcile against the still-present ProxyCTL entry would even
	//     recreate them. Observed after a real delete: a *-egress-wg Secret
	//     reappeared seconds later for a game that no longer existed.
	//
	//     Best-effort and NotFound-tolerant — most instances never had a
	//     tunnel, and delete must stay re-runnable.
	{
		ph := rep.BeginPhase("Remove egress tunnel keys", egressSecretName(name))
		err := b.clientset.CoreV1().Secrets(ns).Delete(ctx, egressSecretName(name), metav1.DeleteOptions{})
		if err != nil && !apierrors.IsNotFound(err) {
			rep.EndPhase(ph, err)
			errs = append(errs, "egress keys: "+err.Error())
		} else {
			rep.EndPhase(ph, nil)
			if err == nil {
				out.Deleted = append(out.Deleted, "Secret/"+egressSecretName(name))
			}
		}
	}
