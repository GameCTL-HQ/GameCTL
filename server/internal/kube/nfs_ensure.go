...
mount -t nfs -o retry=0,timeo=30 "$SERVER":"$ROOT_PARENT" /p 2>/tmp/mount.err
RC=$?
if [ $RC -ne 0 ]; then
...
echo "STEP:mkdir-leaf LEAF=$LEAF"
mkdir -p "/p/$LEAF" 2>/tmp/mkdir.err
RC=$?
if [ $RC -ne 0 ]; then
...
chmod 0777 "/p/$LEAF" 2>/tmp/chmod.err
RC=$?
if [ $RC -ne 0 ]; then
...
touch "/p/$LEAF/$MARKER" 2>/tmp/touch.err
RC=$?
if [ $RC -ne 0 ]; then
...
umount /p
echo "STEP:ok"
`, parent, leaf, ownedMarker, server)
...
func classifyEnsureFailure(server, p string, waitErr error, logs string) string {
	...
	case "touch-marker":
		headline = "Directory exists but couldn't write the .gamectl-owned marker."
		hint = "Export is read-only or out of space. Check `df -h` on the NFS server and confirm the export line in /etc/exports has `rw`."
	default:
...
func (c *Cluster) wipeNFSPath(ctx context.Context, server, p string) error {
	...
	script := fmt.Sprintf(
		"set +e; apk add --no-cache nfs-utils findutils >/dev/null 2>&1; mkdir -p /p; "+
			"mount -t nfs -o retry=0,timeo=30 %s:%s /p || exit $?; "+
