...
func storagePath(vols []corev1.Volume) string {
	// Infrastructure volumes that are never the instance's data, and must be
	// invisible to the fallback scan below.
	//
	// The egress tunnel sidecar mounts /lib/modules as a hostPath. An
	// instance whose data lives on a PVC has no inline nfs/hostPath volume
	// for pick() to match, so the fallback happily returned the FIRST
	// hostPath it found — and reported every egress-published game's storage
	// as "/lib/modules (local)" instead of its actual share. The data was
	// never affected; only what the UI claimed about it.
	infra := func(name string) bool {
		return strings.HasPrefix(name, "egress-")
	}
...
func (c *Cluster) InstanceStatus(ctx context.Context, ns, name string) (InstanceStatus, error) {
	...
	svc := resolveInstanceService(ctx, b, ns, name, dep.Spec.Template.Labels)
	if svc != nil {
...
// resolveInstanceService finds the Service that fronts a Deployment. Tries,
// in order:
...
//
// Shared by InstanceStatus (live view) and the probe-history sampler
// (background polling) so both agree on which Service a game's traffic
// actually goes through.
func resolveInstanceService(ctx context.Context, b *clientBundle, ns, depName string, podLabels map[string]string) *corev1.Service {
	for _, candidate := range []string{depName + "-service", depName} {
