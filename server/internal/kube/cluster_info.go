...
type PortUse struct {
	...
	// Source distinguishes the two port spaces a game occupies, which have
	// very different collision rules:
	//   "service"  — a Kubernetes Service port. Sharing a number is legal;
	//                each Service has its own ClusterIP.
	//   "proxyctl" — a PUBLIC port on the ProxyCTL droplet. One port routes
	//                to exactly one target, so sharing is impossible and the
	//                wizard must treat it as a hard conflict.
	// Empty means "service", so older callers keep working.
