...
type InstanceSettings struct {
	...
	// ServerListing: does this instance register with the GAME's own public
	// server browser (Barotrauma's master, IW4x's dpmaster, WF2's PlayFab,
	// Palworld's community list)? Entirely separate from Advertise above,
	// which only governs GameCTL's Stats API output.
	//
	// Carried here rather than left to the publish endpoint purely so the
	// manage screen can render its switch in the first paint: this handler
	// already reads the Deployment the annotation lives on, whereas
	// /publish additionally round-trips to ProxyCTL for entries, domains and
	// droplet info, which made the switch appear seconds after the rest of
	// the page.
	ServerListing bool `json:"serverListing"`
}
...
func (c *Cluster) InstanceSettings(ctx context.Context, ns, name string) (InstanceSettings, error) {
	...
	out.ServerListing = dep.Annotations[PublishModeAnno] == "egress"
