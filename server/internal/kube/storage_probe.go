...
func (c *Cluster) TestStorageLocation(ctx context.Context, loc StorageLocation) (*StorageProbeResult, error) {
	// Same normalization the save path applies, so testing a path typed with
	// a trailing slash probes the location the operator means instead of
	// failing "not normalized" in setup — and so the probe pod's mount path
	// matches what a real game would get.
	loc = loc.Normalized()
