...
func (c *Cluster) ListBackups(ctx context.Context, ns, name string) ([]BackupArchive, error) {
	...
	// TSV: name<TAB>size<TAB>mtime. Missing dir prints nothing.
	script := fmt.Sprintf(
		"set -e; apk add --no-cache nfs-utils >/dev/null 2>&1; mkdir -p /m; "+
			"mount -t nfs -o nfsvers=4.2,retry=0,timeo=30,ro %s:%s /m; "+
			"d=/m/%s; if [ -d \"$d\" ]; then for f in \"$d\"/%s-*.tar.gz; do [ -e \"$f\" ] || continue; "+
			"printf '%%s\\t%%s\\t%%s\\n' \"$(basename \"$f\")\" \"$(stat -c %%s \"$f\")\" \"$(stat -c %%Y \"$f\")\"; done; fi; "+
			"umount /m",
		dstServer, dstExport, relDir, name,
	)
	out, err := c.runCaptureHelper(ctx, "gamectl-backup-ls", script)
	if err != nil {
		return nil, err
	}
...
// runCaptureHelper runs a short privileged helper pod (mounts NFS in-script)
// and returns its stdout. Mirrors ensureNFSPath's helper-pod pattern but reads
// the logs back before deleting the pod.
func (c *Cluster) runCaptureHelper(ctx context.Context, prefix, script string) (string, error) {
	b := c.snap()
	priv := true
	podName := fmt.Sprintf("%s-%d", prefix, time.Now().UnixNano()/1e6)
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      podName,
			Namespace: nsGamectl,
