...
// volume. A few games' save location is per-instance (an env var picks the
// world/identity name), so those are resolved dynamically here the same way
// minecraft's world dir is.
func effectiveSavePaths(dep *appsv1.Deployment, game, scope string) []string {
	...
	case "necesse":
		world := containerEnv(dep, "WORLD")
		if world == "" {
			world = "GameCTL"
		}
		return []string{world}
	case "unturned":
		id := containerEnv(dep, "SERVER_ID")
		if id == "" {
			id = "gamectl"
		}
		return []string{".gamectl/install/Servers/" + id}
	}
	return spec.SavePaths
}

// effectiveExcludePaths resolves the per-game exclude list for a scope.
// "whole" bypasses per-game curation entirely (that's its point), so excludes
// don't apply there.
func effectiveExcludePaths(game, scope string) []string {
	if scope == "whole" {
		return nil
	}
	return backupSpecFor(game).ExcludePaths
}
...
func (c *Cluster) SetBackupConfig(ctx context.Context, ns, name string, cfg BackupConfig) error {
	...
	if cfg.Enabled {
		...
		// Create GameCTL-Backups/<server> on the share now — and prove it is
		// writable — the same way game data dirs are ensured at apply time.
		// Hard failure on purpose: the backup Job runs as an ordinary NFS
		// client, so if the destination isn't writable (root_squash + a
		// root-owned backups folder is the classic), it's far better to fail
		// here with the classified hint than at 4am in a Job whose busybox
		// mkdir error explains nothing.
		if err := c.ensureNFSPathUnder(ctx, ds, ex, rel); err != nil {
			return fmt.Errorf("backup destination %q is not writable: %w", cfg.Destination, err)
		}
	}
...
func (c *Cluster) reconcileBackupCronJob(ctx context.Context, dep *appsv1.Deployment, cfg BackupConfig) error {
	...
	pod := c.backupPodTemplate(name, game, srcServer, srcPath, dstServer, dstExport, relDir,
		effectiveSavePaths(dep, game, cfg.Scope), effectiveExcludePaths(game, cfg.Scope), cfg.Retention, false, "")
...
func (c *Cluster) backupPodTemplate(server, game, srcServer, srcPath, dstServer, dstExport, relDir string,
	savePaths, excludePaths []string, retention int, restore bool, archive string) corev1.PodTemplateSpec {
		...
		Spec: corev1.PodSpec{
			...
			Containers: []corev1.Container{{
				...
				Env: []corev1.EnvVar{
					...
					{Name: "EXCLUDES", Value: strings.Join(excludePaths, " ")},
...
DST="/dst/$RELDIR"
# mkdir -p over NFS can fail EACCES even when the dir already exists (the
# server checks parent write perms before existence), so tolerate it and let
# the explicit write test below decide.
mkdir -p "$DST" 2>/dev/null || true
if ! touch "$DST/.gamectl-write-test" 2>/dev/null; then
  echo "backup: destination $DST is not writable from this pod — usually root_squash on the NFS export with a root-owned backups folder. Re-save the backup settings in Manage -> Backups to auto-fix permissions, or on the NFS server: chmod -R 0777 <export>/GameCTL-Backups (or export with no_root_squash)." >&2
  exit 3
fi
rm -f "$DST/.gamectl-write-test"
...
if [ $# -eq "$nflags" ]; then echo "backup: no save paths present under /src (looked for: $SAVEPATHS) — try whole-volume scope" >&2; exit 2; fi
...
func (c *Cluster) BackupNow(ctx context.Context, ns, name string, rep PhaseReporter) error {
	...
	// Ensure the destination dir exists and is writable before launching the
	// Job — this also heals dirs an older version left root-owned 0755 (a
	// root_squash'd Job pod can't write into those). Cheap (~5s helper pod)
	// next to a backup run, and it turns "mkdir: Permission denied" into an
	// actionable storage-side hint.
	if err := c.ensureNFSPathUnder(ctx, dstServer, dstExport, relDir); err != nil {
		err = fmt.Errorf("backup destination %q is not writable: %w", cfg.Destination, err)
		rep.EndPhase(p, err)
		return err
	}
	...
	pod := c.backupPodTemplate(name, game, srcServer, srcPath, dstServer, dstExport, relDir,
		effectiveSavePaths(dep, game, cfg.Scope), effectiveExcludePaths(game, cfg.Scope), retain, false, "")
	rep.EndPhase(p, nil)
...
func (c *Cluster) RestoreBackup(ctx context.Context, ns, name, archive string, rep PhaseReporter) error {
	...
	pod := c.backupPodTemplate(name, game, srcServer, srcPath, dstServer, dstExport, relDir, nil, nil, 0, true, archive)
...
// deleteScript removes one archive from the per-server backup dir mounted at
// /dst. Missing file is not an error — deleting an already-gone archive (e.g.
// a double-click) should be a no-op, not a failure the UI has to explain.
const deleteScript = `f="/dst/$RELDIR/$ARCHIVE"
[ -e "$f" ] || { echo "delete: $f already gone"; exit 0; }
rm -f "$f"
echo "delete: removed $f"
`

// DeleteBackup removes one stored archive from the backup destination. Reads
// the destination the same way ListBackups does; write access needs the dst
// volume mounted read-write (ListBackups mounts it read-only).
func (c *Cluster) DeleteBackup(ctx context.Context, ns, name, archive string) error {
	b := c.snap()
	if b == nil {
		return ErrNotConfigured
	}
	...
	pod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("gamectl-backup-rm-%d", time.Now().UnixNano()/1e6),
			Namespace: nsGamectl,
			Labels: map[string]string{
				"app.kubernetes.io/managed-by": "gamectl",
				"app.kubernetes.io/component":  backupComponent,
		...
		Spec: corev1.PodSpec{
			...
			Containers: []corev1.Container{{
				Name:    "rm",
				Image:   backupImage,
				Command: []string{"sh", "-c", deleteScript},
				Env: []corev1.EnvVar{
					{Name: "RELDIR", Value: relDir},
					{Name: "ARCHIVE", Value: archive},
				},
			...
			Volumes: []corev1.Volume{
				{Name: "dst", VolumeSource: corev1.VolumeSource{NFS: &corev1.NFSVolumeSource{Server: dstServer, Path: dstExport}}},
			},
		},
	}
	_, err = c.runCapturePod(ctx, pod)
	return err
}
