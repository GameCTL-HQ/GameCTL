...
func init() {
	...
	// DayZ answers A2S on its steamQueryPort, NOT the game port. The
	// generator pins that value in the config it writes and exposes it as
	// the Service port named "query-udp", so hint the name rather than
	// letting the UDP-only fallback grab game-udp (which never answers).
	RegisterWithHint("dayz", probeSourceA2S, PortHint{Protocol: "UDP", PortName: "query-udp"})
}
