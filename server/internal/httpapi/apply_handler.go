...
// polls via /api/tasks.
func applyManifests(c *kube.Cluster, store *tasks.Store, hub *proxyctlHub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Snapshot the docs against the task ID so /api/tasks/{id}/rerun
		// can replay the exact same payload after a failure. The UI's
		// "Re-run" button uses this — saves the operator from having to
		// walk through the deploy wizard again to fix a transient NFS /
		// kube error.
		...
		go runApplyTask(c, hub, handle, docs)
...
// applyRunner is the adapter rerunTask uses to kick off a new apply
// without reaching into this file's internals. Holds the kube + task
// store handles the worker needs.
type applyRunner struct {
	cluster *kube.Cluster
	hub     *proxyctlHub
}

// RunApply implements the rerunDeps interface declared in tasks_handler.go.
// Just hands off to the package-internal worker.
func (a applyRunner) RunApply(handle ApplyHandle, docs []map[string]any) {
	runApplyTask(a.cluster, a.hub, handle, docs)
}
...
//
// After a successful apply it fulfills any deploy-time ProxyCTL publish
// intent (wizard annotations) as an extra best-effort phase — Services get
// their ClusterIPs at apply time, so the DNS association can ride the same
// task with no waiting.
func runApplyTask(c *kube.Cluster, hub *proxyctlHub, handle interface {
	ID() string
	Start()
	...
	if err == nil && hub != nil {
		publishAfterApply(ctx, hub, handle, docs)
	}
	handle.Finish(err)
}
