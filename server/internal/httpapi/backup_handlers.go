...
// deleteInstanceBackup handles DELETE /games/instances/{ns}/{name}/backups/{archive}
// — removes one stored archive. Synchronous (a single NFS unlink, not worth a
// tracked task).
func deleteInstanceBackup(c *kube.Cluster) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if !kubeRequired(w, c) {
			return
		}
		...
		if err := c.DeleteBackup(r.Context(), ns, name, archive); err != nil {
			code, msg := kube.AsAPIError(err)
			writeError(w, code, msg)
			return
		}
