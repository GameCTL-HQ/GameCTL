...
func NewRouter(authn *auth.Authenticator, cluster *kube.Cluster, cfg *config.Config, taskStore *tasks.Store) http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Public server-BROWSER listing (the game's own master server).
			// Distinct from .../advertise above, which only controls
			// GameCTL's Stats API output.
			r.Patch("/games/instances/{namespace}/{name}/server-listing", setServerListing(proxyctlHub))
