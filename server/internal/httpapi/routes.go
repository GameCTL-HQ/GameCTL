...
func NewRouter(authn *auth.Authenticator, cluster *kube.Cluster, cfg *config.Config, taskStore *tasks.Store) http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Version + self-update (in-place rollout; keeps auth Secret and
			// game servers — see RolloutRestart).
			r.Get("/version", versionHandler)
			r.Get("/update/check", updateCheck(updChecker))
			r.Get("/release-notes", releaseNotes)
			r.Post("/update/apply", updateApply(cluster, cfg))
			...
			r.Group(func(r chi.Router) {
				...
				r.Post("/kube/apply", applyManifests(cluster, taskStore))
				r.Delete("/games/instances/{namespace}/{name}", deleteInstance(cluster, taskStore))
			})
			...
			r.Post("/tasks/{id}/rerun", rerunTask(applyRunner{cluster: cluster, store: taskStore}, taskStore))
