...
// installMetricsServer handles POST /api/cluster/metrics-server/install.
func installMetricsServer(c *kube.Cluster) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if !kubeRequired(w, c) {
			return
		}
		...
		res, err := c.InstallMetricsServer(r.Context())
		if err != nil {
			code, msg := kube.AsAPIError(err)
			writeError(w, code, msg)
			return
		}
		st, _ := c.MetricsServerStatus(r.Context())
