...
// setServerListing turns the game's public server-browser listing on or off
// for an already-deployed instance — the manage-screen equivalent of the
// wizard's egress toggle.
//
// NOT the same thing as the Stats-API export switch next to it ("Export
// this server's stats via the GameCTL API"). That one
// only controls whether GameCTL's own Stats API includes this server in its
// public JSON, for a promo site to render. This one decides whether the GAME
// announces itself to ITS OWN browser (Barotrauma's master, IW4x's dpmaster,
// Wreckfest 2's PlayFab, Palworld's community list) — a completely separate
// system that GameCTL does not operate. The two are independent: a server can
// be listed in-game and absent from your stats page, or the reverse.
//
// Turning it on routes the pod's outbound traffic through ProxyCTL's droplet
// so the address the game's backend records is the one players can reach.
// Turning it off strips the tunnel sidecar and drops the ProxyCTL entry back
// to inbound-only, so the game never registers and receives none of the
// query traffic a public listing attracts.
func setServerListing(hub *proxyctlHub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if !kubeRequired(w, hub.cluster) {
			return
		}
		...
		if err := hub.cluster.SetInstancePublishMode(r.Context(), ns, name, req.Enabled); err != nil {
			writeError(w, http.StatusInternalServerError, err.Error())
			return
		}

		// The annotation alone changes nothing that is already running, so
		// reconcile the live state too. A game that has never been published
		// through ProxyCTL has no entry to update — the annotation is then
		// simply recorded for whenever it is published.
		client, _, err := hub.get(r.Context())
		if err != nil || client == nil {
			writeJSON(w, http.StatusOK, map[string]any{
				"enabled": req.Enabled,
				"note":    "saved — publish this server through ProxyCTL for it to take effect",
			})
			return
		}
		...
		target := findTarget(buildTargets(svcs, rows, webRoutes, ns, name), "")
		...
		note := ""
		if req.Enabled {
			isEgress, excl, err := prepareEgressEntry(r.Context(), hub, ns, name, &e, target.Entry)
			if err != nil {
			...
			if isEgress {
				if note, err = ensureEgressSidecar(r.Context(), hub, client, ns, name, saved, excl); err != nil {
					writeError(w, http.StatusInternalServerError, "egress sidecar: "+err.Error())
					return
				}
			}
		} else {
			// Drop the data plane back to inbound BEFORE removing the
			// sidecar, so the droplet stops SNATing this peer first and we
			// never leave an entry pointing at a tunnel that no longer has a
			// peer on the other end.
			//
			// "inbound", NOT "". ProxyCTL's Entry.Mode is `json:"mode,omitempty"`,
			// so an empty string is omitted from the request body entirely and
			// the saved entry keeps whatever mode it already had. Observed
			// live: the sidecar was removed while the entry stayed egress, so
			// the droplet went on SNATing a peer that no longer existed and
			// the game became unreachable from outside. Both values mean the
			// same data plane to ProxyCTL, but only this one survives the wire.
			e.Mode = "inbound"
			if _, err := client.UpdateEntry(r.Context(), e); err != nil {
				writeError(w, http.StatusBadGateway, err.Error())
				return
			}
			if err := hub.cluster.RemoveEgressSidecar(r.Context(), ns, name); err != nil {
				writeError(w, http.StatusInternalServerError, "egress cleanup: "+err.Error())
				return
			}
			note = "server-list registration disabled — the game pod restarts once, and it will stop appearing in the public browser"
		}
		applyBusy, applyErr := client.Apply(r.Context())
		out := map[string]any{
			"enabled": req.Enabled, "note": note,
			"applied": applyErr == nil && !applyBusy,
		}
		if applyBusy {
			out["applyNote"] = "a ProxyCTL apply was already running — re-apply from ProxyCTL to push this change"
		} else if applyErr != nil {
			out["applyNote"] = applyErr.Error()
		}
...
// createEntryResolvingPorts creates an L4 entry, moving the PUBLIC side of
// any contested port out of the way instead of failing.
//
// ProxyCTL routes one public port to exactly one target, so publishing a
// second game that happens to use a Source-default port (27015) collides with
// whatever got there first. GameCTL used to surface that 409 verbatim and
// stop — the deploy's "Publish via ProxyCTL" phase became a dead end that the
// operator had to go resolve by hand in another app.
//
// ProxyCTL's 409 already carries the nearest free public port, and its
// PortSpec already supports a public→target remap. So: retry with the
// suggested public port, pinned to deliver to the port the game actually
// listens on. Nothing about the game server changes — only the number players
// type. Each retry resolves one port, so the loop is bounded by the port count
// plus a small margin rather than running until ProxyCTL gives up.
//
// Returns the saved entry and a human-readable note per remap (empty when
// everything published on its requested port).
func createEntryResolvingPorts(ctx context.Context, client *proxyctl.Client, e proxyctl.Entry) (proxyctl.Entry, []string, error) {
	var notes []string
	for attempt := 0; attempt <= len(e.Ports)+1; attempt++ {
		saved, err := client.CreateEntry(ctx, e)
		if err == nil {
			return saved, notes, nil
		}
		var pc *proxyctl.PortConflict
		if !errors.As(err, &pc) || pc.Suggested == 0 {
			return proxyctl.Entry{}, notes, err
		}
		moved := false
		for i := range e.Ports {
			if e.Ports[i].Port != pc.Port {
				continue
			}
			// Preserve an existing remap: the in-cluster target is whatever
			// this spec already delivers to, not necessarily its public number.
			target := e.Ports[i].TargetPort
			if target == 0 {
				target = e.Ports[i].Port
			}
			e.Ports[i].Port = pc.Suggested
			e.Ports[i].TargetPort = target
			notes = append(notes, fmt.Sprintf(
				"public %d/%s was taken by %q — published on %d instead (still delivers to %d in-cluster)",
				pc.Port, pc.Proto, pc.Holder, pc.Suggested, target))
			moved = true
			break
		}
		if !moved {
			// The contested port isn't one we asked for — nothing to move.
			return proxyctl.Entry{}, notes, err
		}
	}
	return proxyctl.Entry{}, notes, fmt.Errorf("could not find free public ports after %d attempts", len(e.Ports)+2)
}
...
func publishAfterApply(ctx context.Context, hub *proxyctlHub, rep phaseReporter, docs []map[string]any) {
	...
	for i := range targets {
		...
		saved, remaps, err := createEntryResolvingPorts(ctx, client, e)
		if err != nil {
			if firstErr == nil {
				firstErr = err
			}
		...
		// Surface remaps in the task log: the public port players need is no
		// longer the one on the wizard's Networking step, so silently
		// succeeding would be worse than the old hard failure.
		did = append(did, remaps...)
		if isEgress {
...
func publishInfo(hub *proxyctlHub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Current state of the game's public server-BROWSER listing, so the
		// manage screen can render its toggle without guessing. Read from the
		// live Deployment rather than the entry: the annotation is the source
		// of truth and survives an instance that is not published yet.
		if mode, _, err := hub.cluster.InstancePublishMode(r.Context(), ns, name); err == nil {
			out["serverListing"] = mode == "egress"
		}
		writeJSON(w, http.StatusOK, out)
	}
}
...
func publishSet(hub *proxyctlHub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if existing != nil {
			...
			saved, remaps, err = createEntryResolvingPorts(r.Context(), client, e)
		}
		if err != nil {
			// Hand the UI ProxyCTL's structure, not just its prose, so a
			// collision the auto-remap could not resolve still offers the
			// operator a concrete port to pick instead of a dead end.
			var pc *proxyctl.PortConflict
			if errors.As(err, &pc) {
