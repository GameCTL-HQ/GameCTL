...
// TestCreateEntryResolvingPortsRemapsPublicSide covers the failure the
// operator actually hit: publishing a second game whose port a live entry
// already routes. ProxyCTL answers 409 with a suggestion; GameCTL must move
// the PUBLIC port and pin the in-cluster target, not give up.
func TestCreateEntryResolvingPortsRemapsPublicSide(t *testing.T) {
	var posted []proxyctl.Entry
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/token" {
			_ = json.NewEncoder(w).Encode(map[string]string{"access_token": "t"})
			return
		}
		var e proxyctl.Entry
		_ = json.NewDecoder(r.Body).Decode(&e)
		posted = append(posted, e)
		for _, p := range e.Ports {
			if p.Port == 27015 { // taken by "cs2", as in the live cluster
				w.WriteHeader(http.StatusConflict)
				_ = json.NewEncoder(w).Encode(map[string]any{
					"error": `public port 27015/udp is already routed by enabled entry "cs2"`,
					"conflict": map[string]any{
						"port": 27015, "proto": "udp", "holder": "cs2", "suggested": 27016,
					},
				})
				return
			}
	...
	client := proxyctl.New(srv.URL, "u", "p")
	saved, notes, err := createEntryResolvingPorts(context.Background(), client, proxyctl.Entry{
		Name: "factorio",
		Ports: []proxyctl.PortSpec{
			{Port: 34197, Proto: "udp"},
			{Port: 27015, Proto: "udp"},
		},
	})
	if err != nil {
		t.Fatalf("want the conflict resolved, got error: %v", err)
	}
	if len(posted) != 2 {
		t.Fatalf("want one retry after the 409, got %d POSTs", len(posted))
	}
	// The untouched port keeps its number and stays un-remapped.
	if got := saved.Ports[0]; got.Port != 34197 || got.TargetPort != 0 {
		t.Errorf("uncontested port changed: %+v", got)
	}
	// The contested one moves publicly but still delivers to 27015 in-cluster,
	// so the game server itself needs no reconfiguration.
	if got := saved.Ports[1]; got.Port != 27016 || got.TargetPort != 27015 {
		t.Errorf("want public 27016 → target 27015, got %+v", got)
	}
	if len(notes) != 1 || !strings.Contains(notes[0], "27016") || !strings.Contains(notes[0], "cs2") {
		t.Errorf("want a note naming the new port and the holder, got %q", notes)
	}
}

// A conflict ProxyCTL can't suggest a way out of must still fail rather than
// loop — and must reach the caller as a *PortConflict so the UI can render it.
func TestCreateEntryResolvingPortsGivesUpWithoutSuggestion(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/token" {
			_ = json.NewEncoder(w).Encode(map[string]string{"access_token": "t"})
			return
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			"error": "public port 27015/udp is already routed by enabled entry \"cs2\"",
			"conflict": map[string]any{
				"port": 27015, "proto": "udp", "holder": "cs2", "suggested": 0,
			},
		})
	}))
	defer srv.Close()

	_, _, err := createEntryResolvingPorts(context.Background(), proxyctl.New(srv.URL, "u", "p"),
		proxyctl.Entry{Name: "factorio", Ports: []proxyctl.PortSpec{{Port: 27015, Proto: "udp"}}})
	var pc *proxyctl.PortConflict
	if !errors.As(err, &pc) {
		t.Fatalf("want *proxyctl.PortConflict, got %v", err)
	}
	if pc.Holder != "cs2" || pc.Port != 27015 {
		t.Errorf("conflict structure lost: %+v", pc)
	}
}
