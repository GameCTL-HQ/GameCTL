...
func deleteInstance(c *kube.Cluster, store *tasks.Store, hub *proxyctlHub) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		go func(opts kube.DeleteOptions) {
			...
			// blocks the delete.
			// Egress-published instances are ALWAYS unpublished, whether or
			// not the operator ticked the box.
			//
			// The box is a real choice for an inbound entry: deleting and
			// redeploying under the same name is a normal workflow, and
			// keeping the entry preserves its ports and DNS record. An egress
			// entry is different. It also pins a WireGuard peer and an
			// outbound SNAT rule on the droplet to THIS pod's tunnel identity,
			// and the keys are destroyed with the instance — so what survives
			// is not a reusable binding, it is public ports forwarding to a
			// tunnel with nothing behind it.
			//
			// Seen for real: a deleted Barotrauma left 27017/27022 open on the
			// droplet routing to a Service that no longer existed, and a later
			// reconcile against that orphan recreated the instance's egress
			// keys. Nothing reported an error at any point.
			forced := false
			if !unpublish && hub != nil {
				if mode, _, err := hub.cluster.InstancePublishMode(ctx, ns, name); err == nil && mode == "egress" {
					forced = true
				}
			}
			if (unpublish || forced) && hub != nil {
