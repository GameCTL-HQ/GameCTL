...
func releaseNotes(w http.ResponseWriter, _ *http.Request) {
	...
	// Released (tag-stamped) builds don't list the "Unreleased" staging
	// entry — that's dev-build territory; on a release it's just an empty
	// "changes will appear here" placeholder.
	if releasenotes.IsTagged(buildinfo.Version) {
		kept := make([]releasenotes.Release, 0, len(all))
		for _, r := range all {
			if r.Unreleased || strings.EqualFold(strings.TrimSpace(r.Version), "unreleased") {
				continue
			}
			kept = append(kept, r)
		}
