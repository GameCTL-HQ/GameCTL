...
🌐 **[gamectl.cc](https://gamectl.cc)** &nbsp;·&nbsp; 🚀 **Public beta — `v0.0.33-beta`** &nbsp;·&nbsp; 🔗 **Sibling: [ProxyCTL](https://proxyctl.cc)** — host your servers on the internet, securely
...
> at `v0.0.33-beta`. It's stable enough for day-to-day self-hosting and
...
- **First-party server images** — most titles run GameCTL's own from-scratch images, built in public and published to GHCR
...
- Most supported titles run **GameCTL's own from-scratch server images** —
...
- The remaining SteamCMD titles are being converted; any image can be
  overridden in the wizard
...
**13 games deployable today.** ✦ = runs GameCTL's own first-party image
(`ghcr.io/gamectl-hq/<game>-kube`); the rest are being converted and
currently default to a community image.

| Game | Server image | Status |
|---|---|---|
| Minecraft (Java, modded-friendly, optional BlueMap) | ✦ first-party | ✅ Supported |
| CS2 (Counter-Strike 2, modded — Metamod/CounterStrikeSharp) | community *(conversion in progress)* | ✅ Supported |
| SPT + Fika (Tarkov co-op) | ✦ first-party | ✅ Supported |
| Insurgency: Sandstorm | community *(conversion in progress)* | ✅ Supported |
| Satisfactory | community *(conversion in progress)* | ✅ Supported |
| Valheim | ✦ first-party | ✅ Supported |
| Factorio | ✦ first-party | ✅ Supported |
| 7 Days to Die | community *(conversion in progress)* | ✅ Supported |
| Core Keeper | ✦ first-party | ✅ Supported |
| Terraria | ✦ first-party | ✅ Supported |
| Project Zomboid | ✦ first-party | ✅ Supported |
| Necesse | ✦ first-party | ✅ Supported |
| Barotrauma | ✦ first-party | ✅ Supported |
...
- Finish the **first-party image conversion** (CS2, Satisfactory, 7 Days to Die, Insurgency: Sandstorm)
...
**GameCTL builds and ships its own game-server images.** Most supported
titles run first-party, from-scratch images built in public per-game repos
under [GameCTL-HQ](https://github.com/GameCTL-HQ) and published to GHCR as
`ghcr.io/gamectl-hq/<game>-kube` — maintained by the GameCTL project
itself, end to end.

A few SteamCMD-based titles still default to community images while their
first-party conversions are finished — currently CS2
(`ghcr.io/kus/cs2-modded-server`), Satisfactory
(`wolveix/satisfactory-server`), 7 Days to Die (`didstopia/7dtd-server`),
and Insurgency: Sandstorm (`andrewmhub/insurgency-sandstorm`). Thanks to
those maintainers; any image can be overridden in the wizard. The game
servers themselves and the games remain the property of their respective
owners, governed by their own licenses and each game's EULA.
...
themselves and any remaining community images carry their own licenses
and terms.
...
**Public beta `v0.0.33-beta`** · Active development · Interfaces, templates, and structure subject to change
